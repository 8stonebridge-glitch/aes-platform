...
function evictEventBuffers() {
    if (jobEventBuffer.size <= MAX_EVENT_BUFFERS)
        return;
...
async function pushToConvex(path, body) {
    if (!CONVEX_SITE_URL)
        return;
    try {
...
function syncJobToConvex(jobId) {
    const store = getJobStore();
...
async function readApprovalSignal(jobId) {
    const store = getJobStore();
    if (!store.hasPersistence())
        return null;
    try {
        ...
        for (let i = logs.length - 1; i >= 0; i--) {
            ...
            if (message === APPROVAL_SIGNAL_APPROVED)
                return true;
            if (message === APPROVAL_SIGNAL_REJECTED)
                return false;
        }
    }
    catch {
...
function broadcastToJob(jobId, event, data) {
    const store = getJobStore();
    ...
    if (!jobEventBuffer.has(jobId)) {
        evictEventBuffers();
        jobEventBuffer.set(jobId, []);
    }
...
app.post("/api/build", async (req, res) => {
    ...
    const resolvedDeployTarget = deployTarget === "cloudflare"
        ? "cloudflare"
        : deployTarget === "vercel"
            ? "vercel"
            : "local";
...
app.get("/api/jobs/:id/stream", (req, res) => {
    ...
    res.flushHeaders();
    if (!jobStreams.has(jobId))
        jobStreams.set(jobId, []);
...
app.get("/api/jobs", async (_req, res) => {
    ...
    if (jobs.length === 0 && store.hasPersistence()) {
        ...
        const hydrated = pgJobs.map((j) => ({
            ...
            targetPath: null,
            deployTarget: j.deploy_target || "local",
...
app.get("/api/attention-queue", (_req, res) => {
    ...
    let seq = 0;
    const blocked_builds = [];
    const pending_escalations = [];
    const stale_bridges = [];
    const verified_restricted_write_backs = [];
...
app.get("/api/jobs/:id/features", async (req, res) => {
    ...
    const features = (job.appSpec?.features || []).map((f) => ({
        ...f,
...
app.get("/api/self-audit", async (_req, res) => {
    ...
    `;
        ...
        if (total === 0) {
            ...
            const memSuccesses = allJobs.filter((j) => j.currentGate === "complete").length;
            const memFailures = allJobs.filter((j) => j.currentGate === "failed").length;
            const memRate = memTotal > 0 ? ((memSuccesses / memTotal) * 100).toFixed(1) : "0";
            const memGateBreakdown = allJobs
                .filter((j) => j.currentGate === "failed")
                .reduce((acc, j) => {
    ...
    `;
        ...
        res.json({
            ...
            failures,
            gate_breakdown: gateBreakdown.map((r) => ({ gate: r.gate, category: r.category, count: Number(r.cnt) })),
            suggestions,
        });
    }
    catch (err) {
...
app.get("/api/graph/visualize", async (req, res) => {
    ...
    const limit = Math.min(parseInt(req.query.limit) || 220, 500);
    try {
        const { getNeo4jService } = await import("../services/neo4j-service.js");
        const neo4j = getNeo4jService();
        const ok = await neo4j.connect();
    ...
    `;
        ...
        const rawNodes = connResult[0]?.nodes || [];
        const rawEdges = connResult[0]?.edges || [];
...
app.get("/api/debug/neo4j", async (_req, res) => {
    try {
        ...
        const writeResult = await neo4j.runCypher(`CREATE (p:Probe {id: $id, ts: $ts}) RETURN p.id AS id`, { id: testId, ts: new Date().toISOString() });
        // Test read-back
        const readResult = await neo4j.runCypher(`MATCH (p:Probe {id: $id}) RETURN p.id AS id, p.ts AS ts`, { id: testId });
        // Count all PipelineOutcome nodes
        const outcomeCount = await neo4j.runCypher(`MATCH (o:PipelineOutcome) RETURN count(o) AS cnt`);
        // List recent outcomes
        const recentOutcomes = await neo4j.runCypher(`MATCH (o:PipelineOutcome) RETURN o.job_id AS job, o.success AS success, o.gate_reached AS gate ORDER BY o.created_at DESC LIMIT 5`);
        // Cleanup
...
async function initializePersistenceOnBoot() {
    const store = getJobStore();
    if (store.hasPersistence())
        return;
    const pgUrl = process.env.AES_POSTGRES_URL;
