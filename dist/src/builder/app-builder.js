...
function templateSidebar(appSpec) {
...
function templateDashboard(appSpec) {
    const features = appSpec?.features || [];
...
function templateUnifiedSchema(appSpec) {
    const features = appSpec?.features || [];
...
export class AppBuilder {
    ...
    async buildApp(jobId, appSpec, featureBridges, featureBuildOrder, callbacks, targetPath, reusableSourceFiles, graphContext) {
        ...
        try {
            ...
            for (let i = 0; i < featureBuildOrder.length; i++) {
                ...
                try {
                    ...
                    const featureResult = await this.buildFeatureInPlace(workspace.path, pkg, builderContext, fileContents);
        ...
        run.file_contents = fileContents;
        const prSummary = this.workspaceManager.generatePRSummary(workspace, appSpec?.title || "Application", appSpec?.summary || "Complete application build");
    ...
    async generateAppLevelFiles(basePath, appSpec, fileContents) {
    ...
    async generateLayout(basePath, appSpec, fileContents) {
     ...
     */
    async buildFeatureInPlace(workspacePath, pkg, context, fileContents) {
        ...
        const archetype = matchFeatureArchetype(pkg.feature_name, pkg.objective, pkg.included_capabilities);
        if (archetype) {
        ...
        if (!hasQueries || !hasMutations) {
            ...
            if (!hasMutations)
                filesCreated.push(join("convex", featureSlug, "mutations.ts"));
        }
        ...
        filesCreated.push(...allFiles.filter((f) => !filesCreated.includes(f)));
        // Merge into caller's fileContents
        if (fileContents) {
            Object.assign(fileContents, localContents);
        }
