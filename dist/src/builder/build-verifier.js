...
import { validateCatalogUsage } from "../validators/catalog-usage-validator.js";
import { validateComposition } from "../validators/composition-validator.js";
export function verifyBuild(jobId, pkg, run) {
    ...
    const missingTests = requiredTests.filter(rt => !ranTests.find(t => t.test_id === rt.test_id));
    const failedTests = ranTests.filter(t => !t.passed);
    if (missingTests.length > 0) {
        constraintViolations.push(`Missing required tests: ${missingTests.map(t => t.test_id).join(", ")}`);
    }
    if (failedTests.length > 0) {
        constraintViolations.push(`Failed tests: ${failedTests.map(t => t.test_id).join(", ")}`);
    }
    ...
    if (coverage && coverage.total_required > 0) {
        ...
        if (ratio < 1.0) {
            constraintViolations.push(`Acceptance coverage: ${coverage.covered}/${coverage.total_required} (${(ratio * 100).toFixed(0)}%). Missing: ${(coverage.missing || []).join(", ")}`);
        }
    }
    ...
    if (builtFiles.length > 0) {
        ...
        if (catalogResult.verdict === "FAIL") {
            for (const v of catalogResult.violations) {
                if (v.severity === "error") {
                    constraintViolations.push(`${GateErrorCode.G2_RAW_PRIMITIVE_VIOLATION}: ${v.violation} in ${v.file}:${v.line} — ${v.expected}`);
                }
            }
            // Check for missing required packages
            const missingPkgs = catalogResult.violations.filter(v => v.violation.startsWith("Required package"));
            if (missingPkgs.length > 0) {
                constraintViolations.push(`${GateErrorCode.G2_CATALOG_ASSETS_NOT_USED}: ${missingPkgs.map(v => v.violation).join("; ")}`);
            }
        }
    }
    // 7. Composition validation — check built pages conform to expected patterns (Layer 4)
    let compositionResult;
    const patternRequirements = pkg.pattern_requirements || [];
    if (builtFiles.length > 0 && patternRequirements.length > 0) {
        ...
        if (compositionResult.verdict === "FAIL") {
            for (const v of compositionResult.violations) {
                if (v.severity === "error") {
                    constraintViolations.push(`${GateErrorCode.G2_PATTERN_CONFORMANCE_FAIL}: [${v.pattern}] ${v.category}/${v.check} — ${v.description} in ${v.file}`);
                }
            }
        }
    }
 ...
 */
export function createCheckFixTrailEntries(jobId, runId, bridgeId, checkResults) {
    const entries = [];
