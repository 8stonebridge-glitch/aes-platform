...
import { generateConvexSchema, generateConvexQueries, generateConvexMutations, generatePage, generateComponent, generateTest, } from "../llm/code-gen.js";
...
export class CodeBuilder {
    ...
    async build(jobId, pkg, repoUrl, context) {
    ...
    trackFile(fileContents, basePath, relPath) {
        try {
    ...
    // ─── Convex Schema ──────────────────────────────────────────────────
    async writeConvexSchema(basePath, featureSlug, pkg, feature, appSpec) {
...
`;
    ...
    // ─── Convex Functions ──────────────────────────────────────────────
    async writeConvexFunctions(basePath, featureSlug, pkg, feature, appSpec, schemaContent) {
        const tableName = featureSlug.replace(/-/g, "_");
        // ── Queries ──
...
`;
    ...
    async writeFormPage(basePath, featureSlug, capSlug, cap, pkg, feature, appSpec, fileContents) {
        ...
        if (feature && appSpec) {
            ...
            if (content)
                content = ensureAesUiImports(normalizeClerkUseAuthBindings(ensureClientComponent(content)));
        }
        // Fallback to template
        if (!content) {
