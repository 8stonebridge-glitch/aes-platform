 ...
 */
export function runPreflight(featureId, bridge, classConfig, completedFeatures, workspacePath) {
    ...
    if (classConfig.requires_isolation) {
        const hasAuth = bridge.build_scope.included_capabilities.some(c => c.toLowerCase().includes("auth") || c.toLowerCase().includes("permission"));
        checks.push({
 ...
 */
export function runPreflightAll(featureIds, bridges, classConfigs, completedFeatures, workspacePath) {
    const results = [];
    const ready = [];
    const blocked = [];
