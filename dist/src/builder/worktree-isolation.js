 ...
 */
export function createWorktreePool(jobId, repoUrl) {
 ...
 */
export function createWorktree(pool, featureId, featureName) {
    ...
    execSync(`git worktree add -b ${branch} "${worktreePath}" ${pool.integration_branch}`, { cwd: pool.base_repo_path, stdio: "pipe" });
 ...
 */
export function mergeWorktree(pool, featureId, commitMessage) {
 ...
 */
export function cleanupWorktree(pool, featureId) {
    const worktree = pool.active_worktrees.get(featureId);
    if (!worktree)
        return;
    try {
 ...
 */
export function cleanupPool(pool) {
 ...
 */
export function getWorktreeDiff(worktree) {
    try {
 ...
 */
export function getIntegrationResult(pool) {
