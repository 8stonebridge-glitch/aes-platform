...
export async function logsCommand(jobId, options) {
    ...
    let filtered = logs;
    if (options.gate) {
        filtered = filtered.filter((l) => l.gate === `gate_${options.gate}`);
    }
    if (options.feature) {
        filtered = filtered.filter((l) => (l.featureId || l.feature_id) === options.feature);
    }
