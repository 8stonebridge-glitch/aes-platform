...
import { logHeader, logKeyValue, logFeatureStatus, logDivider, logSuccess, logFail, logWarn, logStep, logGate, } from "../logger.js";
 ...
 */
export async function replayCommand(jobId) {
    ...
    if (job.featureBridges && Object.keys(job.featureBridges).length > 0) {
        ...
        for (const [fId, bridge] of Object.entries(job.featureBridges)) {
            if (!bridge.bridge_id)
                continue;
    ...
    if (job.vetoResults && job.vetoResults.length > 0) {
        ...
        const triggered = job.vetoResults.filter((v) => v.triggered);
        const clean = job.vetoResults.filter((v) => !v.triggered);
        if (triggered.length > 0) {
            for (const v of triggered) {
                logFail(`${v.code}: ${v.reason}`);
            }
        }
