...
export async function viewCommand(jobId) {
    ...
    if (job.appSpec) {
        ...
        console.log(`  ${chalk.gray("Roles:")}          ${spec.roles?.length || 0} (${(spec.roles || []).map((r) => r.name).join(", ")})`);
    ...
    if (job.featureBridges && Object.keys(job.featureBridges).length > 0) {
        const bridges = Object.values(job.featureBridges);
    ...
    if (fixes.length > 0) {
        for (const f of fixes) {
            const statusColor = f.status === "repaired" ? chalk.green :
...
export async function summaryCommand() {
    ...
    for (const job of jobs.slice(0, 20)) {
        ...
        const created = job.createdAt
            ...
            : "unknown";
        const statusColor = gate.includes("fail") ? chalk.red :
