...
export class FrameworkValidator {
    ...
    checkConvexOrgFiltering(base) {
        ...
        this.walkDir(convexDir, (filePath) => {
            if (!filePath.endsWith(".ts") || filePath.includes("_generated") || filePath.includes("tsconfig") || filePath.endsWith("schema.ts"))
                return;
            const content = readFileSync(filePath, "utf-8");
    ...
    checkAuditLogging(base) {
        ...
        const missing = [];
        if (!hasAuditTable)
            missing.push("audit_logs table in schema");
        if (!hasAuditMutation)
            missing.push("convex/audit.ts mutation");
