...
function routeAfterConfirmer(state) {
    if (!state.intentConfirmed)
        return "failure_recorder";
    return "research"; // Research before decomposition
}
function routeAfterSpecValidator(state) {
    const failed = (state.specValidationResults || []).filter((r) => !r.passed);
    if (failed.length > 0) {
        // Retry or fail
        if ((state.specRetryCount || 0) >= 3)
            return "failure_recorder";
        return "decomposer"; // Re-derive
    }
    return "user_approval";
}
function routeAfterUserApproval(state) {
    if (!state.userApproved)
        return "failure_recorder";
    return "catalog_searcher";
}
function routeAfterVetoChecker(state) {
    if (state.errorMessage)
        return "failure_recorder";
    return "graph_updater_pre_build";
}
function routeAfterBuilderDispatcher(state) {
    if (state.errorMessage)
        return "failure_recorder";
    return "validator_runner";
}
function routeAfterValidatorRunner(state) {
    if (state.errorMessage)
        return "failure_recorder";
    return "deployment_handler";
}
function routeAfterDeploymentHandler(state) {
    if (state.errorMessage && state.currentGate === "failed")
        return "failure_recorder";
...
export async function runGraph(input, callbacks) {
