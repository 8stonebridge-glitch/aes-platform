...
export const IntentBriefSchema = z.object({
    ...
    inferred_risk_class: z
        ...
        .describe("Risk classification. Use 'regulated' for anything involving money, banking, or financial compliance"),
    inferred_integrations: z
        .array(z.enum(["payments", "email", "sms", "storage", "maps", "analytics"]))
    ...
    ambiguity_flags: z
        .array(z.string())
        .describe("Areas where the request is unclear. Empty array if the intent is clear"),
    assumptions: z
        .array(z.string())
        .describe("Assumptions made to fill gaps in the request"),
    confirmation_statement: z
        .string()
        .describe("A human-readable summary of the classified intent for user confirmation"),
});
// ─── AppSpec Sub-schemas ──────────────────────────────────────────────────
