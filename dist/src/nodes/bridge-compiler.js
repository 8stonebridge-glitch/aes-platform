...
import { FEATURE_TO_PATTERN, PAGE_PATTERNS } from "../types/pattern-requirements.js";
import { enrichBridgeWithMath, analyzeDependencies, rankPriorities, } from "@aes/math";
import { extractDesignConstraintsForFeature } from "../services/design-evidence-loader.js";
export function resolveReuseRequirements(feature) {
    const requirements = [];
 ...
 */
export function resolvePatternRequirements(feature) {
    const patterns = [];
    const seen = new Set();
...
function compileBridge(feature, appSpec, catalogMatches, buildIndex) {
    ...
    let relevantTests = (appSpec.acceptance_tests || [])
        .filter((t) => t.feature_id === feature.feature_id)
        .map((t) => ({
        test_id: t.test_id,
        name: t.name,
        type: t.type,
    ...
    const deps = (appSpec.dependency_graph || [])
        .filter((e) => e.from_feature_id === feature.feature_id)
        .map((e) => ({
        dependency_id: e.to_feature_id,
        feature_id: e.to_feature_id,
        reason: e.reason,
        status: allFeatureIds.has(e.to_feature_id) ? "satisfied" : "blocked",
    }));
    ...
    const confidence = {
        ...
        dependency_clarity: deps.every((d) => d.status !== "blocked") ? 0.9 : 0.4,
    ...
    return {
        ...
        success_definition: {
            ...
            validation_requirements: relevantTests.map((t) => t.pass_condition),
        },
        confidence,
...
export function validateBridge(bridge) {
    ...
    const overlapping = forbidden.filter((p) => allowed.has(p));
    results.push({
    ...
    const unresolved = deps.filter((d) => d.status !== "satisfied" && d.status !== "blocked");
    results.push({
    ...
    const unresolvable = selectedReuse.filter((id) => !candidateIds.has(id));
    results.push({
    ...
    const triggeredVetoes = vetoes.filter((v) => v.triggered);
    results.push({
...
export async function bridgeCompiler(state) {
    const cb = getCallbacks();
    const store = getJobStore();
    if (!state.appSpec || !state.userApproved) {
        cb?.onFail("Cannot compile bridges — app not approved");
        return { currentGate: "failed", errorMessage: "App not approved" };
    }
    ...
    const depNodes = features.map((f) => ({
        ...
        dependencies: depGraph
            .filter((e) => e.from_feature_id === f.feature_id)
            .map((e) => e.to_feature_id),
    }));
    ...
    for (let i = 0; i < features.length; i++) {
        ...
        const g2Failures = g2Results.filter((r) => !r.passed);
        if (g2Failures.length > 0) {
            bridge.status = "blocked";
            bridge.blocked_reason = g2Failures.map((r) => `${r.code}: ${r.reason}`).join("; ");
        ...
        const deps = bridge.dependencies || [];
        const resolvedDeps = deps.filter((d) => d.status === "satisfied").length;
        ...
        const vetoInput = {
            ...
            unresolved_dependencies: deps.filter((d) => d.status !== "satisfied" && d.status !== "blocked").length,
            scope_violations: [],
        ...
        const matchingPriorBridge = reusableBridges.find((rb) => {
            ...
            return featureWords.some((w) => rbName.includes(w));
        });
        if (matchingPriorBridge) {
            bridge.prior_bridge_id = matchingPriorBridge.bridge_id;
            bridge.confidence.notes.push(`Boosted by prior bridge: ${matchingPriorBridge.feature_name}`);
            bridge.confidence.reuse_fit = Math.min(bridge.confidence.reuse_fit + 0.2, 1.0);
            bridge.confidence.overall = (bridge.confidence.scope_clarity +
                bridge.confidence.reuse_fit +
                bridge.confidence.dependency_clarity +
                bridge.confidence.rule_coverage +
                bridge.confidence.test_coverage) / 5;
        }
        // Check if prior feature spec exists — reduces scope uncertainty
        const matchingPriorFeature = priorFeatures.find((pf) => {
            const pfName = (pf.name || "").toLowerCase();
            return featureWords.some((w) => pfName.includes(w));
        });
        if (matchingPriorFeature) {
            bridge.prior_feature_id = matchingPriorFeature.id;
            bridge.confidence.notes.push(`Informed by prior feature spec: ${matchingPriorFeature.name} (v${matchingPriorFeature.version || 1})`);
        ...
        if (state.designEvidence) {
            ...
            if (dc) {
                ...
                bridge.confidence.notes.push(`Design evidence: ${dc.required_screens.length} screens, ${dc.required_components.length} components`);
        ...
        for (const model of learnedModels) {
            ...
            if (featureWords.some((w) => modelName.includes(w)) ||
                featureWords.some((w) => (model.description || "").toLowerCase().includes(w))) {
                const fields = Array.isArray(model.fields)
                    ? model.fields.map((f) => typeof f === "string" ? f : `${f.name}: ${f.type}`).join(", ")
                    : String(model.fields || "");
        ...
        for (const integration of learnedIntegrations) {
            ...
            if (featureWords.some((w) => intName.includes(w) || intType.includes(w)) ||
                featureWords.some((w) => (integration.description || "").toLowerCase().includes(w))) {
        ...
        for (const rule of preventionRules) {
            ...
            const isRelevant = featureWords.some((w) => ruleName.includes(w) || ruleCondition.includes(w) || ruleAction.includes(w));
        ...
        if (unifiedDomainSources.length > 0) {
            ...
            for (const ds of unifiedDomainSources) {
                ...
                const score = featureWords.reduce((acc, w) => acc + (domainText.includes(w) ? 1 : 0), 0);
                if (score > bestScore) {
                    bestScore = score;
                    bestMatch = ds;
                }
        ...
        for (const pc of graphHints.prevention_constraints) {
            if (pc.severity === "critical" || pc.severity === "error") {
                ...
                // Avoid duplicates
                if (!bridge.applied_rules.some((r) => r.rule_id === ruleId)) {
        ...
        const graphHintCount = graphHints.relevant_models.length +
            graphHints.proven_models.length +
            graphHints.relevant_integrations.length +
            (graphHints.domain_reference ? 1 : 0);
        if (graphHintCount > 0) {
            bridge.confidence.notes.push(`Graph hints: ${graphHints.relevant_models.length} models, ${graphHints.proven_models.length} proven models, ` +
        ...
        cb?.onFeatureStatus(feature.feature_id, feature.name, bridge.status);
        cb?.onStep(`${feature.name}: confidence ${(mathFields.confidence_score * 100).toFixed(1)}% | risk ${(mathFields.risk_score * 100).toFixed(1)}% | priority #${mathFields.priority_rank}${g2Failures.length > 0 ? ` [BLOCKED: ${g2Failures.length} G2 failures]` : ""}`);
    }
    ...
    const candidates = features.map((f) => {
        ...
        const fDeps = depGraph.filter((e) => e.from_feature_id === f.feature_id);
        const allDepsExist = fDeps.every((e) => featureIdSet.has(e.to_feature_id));
