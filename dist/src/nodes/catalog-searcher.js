...
function loadCatalogFromDir(dir) {
    if (!existsSync(dir))
        return [];
 ...
 */
async function fetchSourceFiles(selectedMatches) {
    ...
    for (const match of selectedMatches) {
        if (!match.source_path || match.source_repo === "neo4j-graph")
            continue;
        try {
...
function matchFeatureToCatalog(feature, catalog) {
    ...
    for (const entry of catalog) {
        ...
        const tagOverlap = (entry.tags || []).filter((t) => featureWords.some((w) => t.toLowerCase().includes(w) || w.includes(t.toLowerCase())));
        if (tagOverlap.length > 0) {
            score += 0.3 * (tagOverlap.length / Math.max(featureWords.length, 1));
            reasons.push(`Tag overlap: ${tagOverlap.join(", ")}`);
        }
        // Keyword matching in description
        const descWords = featureWords.filter((w) => w.length > 3);
        const descMatches = descWords.filter((w) => entry.description.toLowerCase().includes(w));
        ...
        if (entry.promotion_tier === "CANONICAL")
            score += 0.1;
        else if (entry.promotion_tier === "VERIFIED")
            score += 0.05;
        if (score > 0.2) {
 ...
 */
export async function catalogSearcher(state) {
    const cb = getCallbacks();
    const store = getJobStore();
    if (!state.appSpec) {
        cb?.onFail("No AppSpec for catalog search");
        return { currentGate: "failed", errorMessage: "Missing AppSpec" };
    }
    ...
    for (const feature of state.appSpec.features) {
        ...
        const featureLower = feature.name.toLowerCase();
        const featureWords = featureLower.split(/[\s-_]+/).filter((w) => w.length > 2);
        for (const pattern of graphPatterns) {
            const nameLower = (pattern.name || "").toLowerCase();
            const wordOverlap = featureWords.filter((w) => nameLower.includes(w));
        ...
        for (const bridge of graphBridges) {
            ...
            const wordOverlap = featureWords.filter((w) => featNameLower.includes(w));
            if (wordOverlap.length > 0) {
                candidates.push({
        ...
        if (candidates.length > 0) {
            ...
            cb?.onStep(`${feature.name}: ${candidates.length} candidates (${bestLabel})`);
        }
    }
    ...
    const uniqueCandidates = Array.from(new Map(allSelectedCandidates.map((c) => [c.candidate_id, c])).values());
    let reusableSourceFiles = {};
