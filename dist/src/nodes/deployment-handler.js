...
import { VercelService, isVercelConfigured, } from "../services/vercel-service.js";
import { repairFilesForCompilerErrors } from "../llm/compiler-repair.js";
import { deployToCloudflare, deployViaApi, } from "../deploy/cloudflare-deploy.js";
...
const CLIENT_HOOK_PATTERN = /\b(useParams|useRouter|usePathname|useSearchParams|useAuth|useQuery|useMutation|usePaginatedQuery|useAction|useState|useEffect|useLayoutEffect|useTransition|useOptimistic)\b/;
const HERMES_REPAIR_URL = (process.env.HERMES_RELEASE_URL ||
...
function normalizeUnsupportedButtonLinkProps(content) {
    ...
    next = next.replace(/<Button([^>]*)>([\s\S]*?)<\/Button>/g, (fullMatch, rawAttrs, children) => {
    ...
    next = next.replace(/<Button([^>]*)\/>/g, (fullMatch, rawAttrs) => {
        const attrs = String(rawAttrs);
        const asAnchor = /\bas=["']a["']/.test(attrs);
        const hrefMatch = attrs.match(/\bhref=["']([^"']+)["']/);
        if (!asAnchor || !hrefMatch)
...
function ensureClientDirective(content) {
...
function ensureRouterBindings(content) {
    ...
    if (existingImport) {
        ...
        if (!names.includes("useRouter")) {
            ...
            next = next.replace(nextNavigationImport, `import { ${merged.join(", ")} } from "next/navigation";`);
        }
    }
    else {
    ...
    if (!/\bconst\s+router\s*=\s*useRouter\(\)\s*;/.test(next)) {
        next = next.replace(/(export default function [^{]+\{\n)/, `$1  const router = useRouter();\n`);
    }
    return { content: next, changed: next !== content };
}
function ensureAesUiImports(content) {
    const required = AES_UI_COMPONENTS.filter((name) => new RegExp(`<${name}\\b`).test(content));
...
function normalizeClerkUseAuthBindings(content) {
    ...
    let next = content.replace(/const\s*{\s*([^}]*)\borg\b([^}]*)}\s*=\s*useAuth\(\)\s*;/g, (_match, before, after) => {
...
function normalizeConvexHandlerBindings(content) {
    let next = content.replace(/handler:\s*async\s*\(\s*ctx\s*,\s*{([^}]*)}\s*\)\s*=>\s*{/g, (_match, bindings) => {
...
function repairLikelyJsxTestFiles(workspacePath, compilerOutput) {
    const renamed = [];
...
function repairLikelyMissingPageImports(workspacePath, compilerOutput) {
    const rewritten = [];
...
function canResolveGeneratedTestImport(workspacePath, testFilePath, specifier) {
    const candidates = [];
    if (specifier.startsWith("@/app/") || specifier.startsWith("@/components/")) {
        const relative = specifier.slice(2);
        const base = join(workspacePath, relative);
        candidates.push(base, `${base}.ts`, `${base}.tsx`, join(base, "index.ts"), join(base, "index.tsx"), join(base, "page.tsx"));
    }
...
function repairBrokenGeneratedTestImports(workspacePath) {
    const rewritten = [];
    for (const absolutePath of collectSourceFiles(join(workspacePath, "tests"), [".ts", ".tsx"])) {
        ...
        const content = readFileSync(absolutePath, "utf-8");
        const imports = Array.from(content.matchAll(/from\s+["']([^"']+)["']/g), (match) => match[1]);
        const broken = imports.find((specifier) => (specifier.startsWith("@/app/") ||
...
function repairTestingLibraryFireEventImports(workspacePath) {
    const rewritten = [];
    for (const absolutePath of collectSourceFiles(join(workspacePath, "tests"), [".ts", ".tsx"])) {
        const relativePath = absolutePath.replace(`${workspacePath}/`, "");
        const existing = readFileSync(absolutePath, "utf-8");
        ...
        if (/@testing-library\/react/.test(repaired) && /\bfireEvent\b/.test(repaired)) {
            repaired = repaired.replace(/import\s*{\s*([\s\S]*?)\s*}\s*from\s*["']@testing-library\/react["'];?/m, (_full, names) => {
                const parsed = String(names)
                    .split(",")
                    .map((name) => name.replace(/\s+/g, " ").trim())
                    .filter(Boolean);
                if (!parsed.includes("fireEvent"))
                    return _full;
        ...
        if (/\brender\s*\(/.test(repaired) && !/\bimport\s*{\s*[^}]*\brender\b[^}]*}\s*from\s*["']@testing-library\/react["']/.test(repaired)) {
            ...
            const reactImportIndex = lines.findIndex((line) => /from\s*["']@testing-library\/react["']/.test(line));
            if (reactImportIndex >= 0) {
                const match = lines[reactImportIndex].match(/import\s*{\s*([^}]*)\s*}\s*from\s*["']@testing-library\/react["'];?/);
                if (match) {
                    const merged = Array.from(new Set(match[1]
                        .split(",")
                        .map((name) => name.trim())
                        .filter(Boolean)
                        .concat("render"))).sort();
...
function repairLikelyUnsupportedTestingLibraryImports(workspacePath, compilerOutput) {
    const rewritten = [];
...
function repairLikelyMissingTestFixtures(workspacePath, compilerOutput) {
    const rewritten = [];
...
function repairLikelyImplicitAnyParameters(workspacePath, compilerOutput) {
    ...
    for (const match of matches) {
        ...
        const params = grouped.get(relativePath) ?? new Set();
    ...
    for (const [relativePath, parameterNames] of grouped.entries()) {
        ...
        for (const parameterName of parameterNames) {
            ...
            next = next.replace(new RegExp(`\\(\\s*${escaped}\\s*\\)(\\s*=>)`, "g"), `(${parameterName}: any)$1`);
            next = next.replace(new RegExp(`async\\s*\\(\\s*${escaped}\\s*\\)(\\s*=>)`, "g"), `async (${parameterName}: any)$1`);
        }
        if (next !== original) {
...
function enforceSourceGuardrailsInWorkspace(workspacePath) {
    const roots = [
...
function commitWorkspaceChanges(workspacePath, message) {
...
function commitWorkspaceSafeguards(workspacePath, files) {
    if (files.length === 0)
        return;
    commitWorkspaceChanges(workspacePath, "[AES] fix: apply generated source guardrails before deploy");
}
...
function findFailingCheck(results) {
    return results.find((result) => !result.passed && !result.skipped) ?? null;
}
function isCompilerNoiseLine(line) {
    const trimmed = line.trim();
    if (!trimmed)
        return true;
    return (/^npm warn config production\b/i.test(trimmed) ||
...
function compilerSignalLines(output) {
    const lines = output
        .split("\n")
...
function summarizeCompilerOutput(output, maxLines = 80) {
    const lines = compilerSignalLines(output);
    return lines.slice(-maxLines).join("\n").trim();
}
function extractPrimaryCompilerPattern(output) {
    const lines = compilerSignalLines(output)
        .map((line) => line.trim())
        .filter(Boolean);
    const preferred = lines.find((line) => /Type error:|Module not found|Can't resolve|Cannot find module|Cannot find name|Property .* does not exist|error TS\d+|Build failed|Expected \d+ arguments|No overload matches this call/i.test(line));
...
function categorizeCompilerPattern(pattern) {
    if (/Module not found|Can't resolve|Cannot find name/i.test(pattern))
        return "module";
    if (/Property .* does not exist|error TS\d+|Type error/i.test(pattern))
        return "typescript";
    if (/useAuth|orgId|Clerk/i.test(pattern))
        return "auth";
    if (/useParams|useRouter|usePathname|useQuery|useMutation|use client/i.test(pattern))
        return "codegen";
    if (/next build|Build failed/i.test(pattern))
        return "build";
    return "compile_gate";
}
async function recallHermesRepairHints(errorMessage) {
    if (!HERMES_REPAIR_URL)
        return [];
    try {
...
async function runPredeployCompileGate(args) {
    ...
    if (preflightRewrittenTests.length > 0) {
        commitWorkspaceChanges(workspacePath, "[AES] fix: rewrite broken generated test imports before compile");
        store.addLog(jobId, {
            gate: "deploying",
    ...
    if (preflightTestingLibraryFixes.length > 0) {
        commitWorkspaceChanges(workspacePath, "[AES] fix: normalize testing-library fireEvent imports before compile");
        store.addLog(jobId, {
            gate: "deploying",
    ...
    for (let attempt = 0; attempt <= PREDEPLOY_MAX_REPAIR_ATTEMPTS; attempt += 1) {
        ...
        const deterministicFiles = deterministicPatches.map((entry) => entry.file.replace(`${workspacePath}/`, ""));
...
export async function deploymentHandler(state) {
    ...
    store.addLog(state.jobId, {
        gate: "deploying",
        message: `Starting deployment via ${state.deployTarget === "cloudflare"
            ? "cloudflare"
        ...
        if (patchedFiles.length > 0) {
            ...
            store.recordHermesReleaseEvent({
                ...
                payload: {
                    ...
                    files: relativeFiles,
                    contract_packs: Array.from(new Set(patchedFiles.flatMap((entry) => entry.packIds ?? []))),
                    contract_rules: Array.from(new Set(patchedFiles.flatMap((entry) => entry.appliedRules ?? []))),
                },
            });
    ...
    const skipRemoteDeployForLocal = state.deployTarget === "local" &&
    ...
    const appDescription = state.appSpec?.summary ||
    ...
    if (state.deployTarget === "cloudflare") {
        if (!workspacePath) {
            ...
            return { currentGate: "failed", errorMessage: "No workspace path for Cloudflare deploy" };
        }
    ...
    if (!hasGithub && !hasVercel) {
        cb?.onWarn("No deployment services configured (GITHUB_TOKEN / VERCEL_TOKEN). Build complete but not deployed.");
        store.addLog(state.jobId, {
            gate: "deploying",
            message: "Deployment skipped: no GitHub or Vercel credentials configured",
        });
        cb?.onSuccess("Build complete. App ready in workspace but not deployed (set GITHUB_TOKEN and VERCEL_TOKEN to enable).");
        return {
            currentGate: "complete",
    ...
    let githubRepoOwnerLogin = null;
    let githubRepoOwnerId = null;
    ...
    if (hasGithub) {
        try {
            ...
            if (workspacePath) {
                ...
                cb?.onWarn("Workspace path not available in build result — cannot push to GitHub");
            }
        }
        catch (err) {
    ...
    if (hasVercel &&
        ...
        try {
            ...
            const clerkPublishableKey = sanitizeEnvValue(process.env.AES_CLERK_PUBLISHABLE_KEY);
            if (clerkPublishableKey) {
                envVars["NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY"] =
                    clerkPublishableKey;
            }
            const clerkSecretKey = sanitizeEnvValue(process.env.AES_CLERK_SECRET_KEY);
            if (clerkSecretKey) {
    ...
    const parts = [];
    if (githubRepoUrl)
        parts.push(`GitHub: ${githubRepoUrl}`);
    if (deploymentUrl)
        parts.push(`Live: ${deploymentUrl}`);
    if (parts.length === 0)
        parts.push("Build complete (no deployment configured)");
