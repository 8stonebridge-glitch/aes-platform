...
// ─── Main designer node ──────────────────────────────────────────
export async function designer(state) {
    const cb = getCallbacks();
    const store = getJobStore();
    // Skip if design evidence already loaded (from Paper MCP or prior extraction)
    if (state.designEvidence) {
        const existing = state.designEvidence;
        cb?.onStep(`Design evidence already loaded (${existing.screens.length} screens, ${existing.components.length} components) — skipping auto-design`);
    ...
    if (state.designMode === "paper") {
        ...
        cb?.onPause(`Design brief ready — ${brief.screens.length} screens, ${brief.directions.length} directions. ` +
        ...
        const designEvidence = await new Promise((resolve) => {
        ...
        cb?.onSuccess(`Design evidence received: ${designEvidence.screens?.length || 0} screens, ${designEvidence.components?.length || 0} components`);
    ...
    if (isLLMAvailable()) {
        try {
            ...
            cb?.onSuccess(`LLM design: ${designEvidence.screens.length} screens, ${designEvidence.components.length} components, ${designEvidence.forms.length} forms`);
        }
        catch (err) {
    ...
    try {
        ...
        await writeFile(join(PLATFORM_ROOT, filename), JSON.stringify(designEvidence, null, 2));
...
async function llmDesign(appSpec, intentBrief) {
    const llm = getLLM();
    const features = (appSpec.features || [])
        .map((f) => `- ${f.name} (${f.feature_id}): ${f.description || f.outcome || ""}`)
        .join("\n");
    const roles = (appSpec.roles || [])
        .map((r) => `- ${r.name} (${r.role_id}): ${r.scope || ""}`)
        .join("\n");
    const result = await safeLLMCall("designer", () => llm.invoke([
...
function templateDesign(appSpec) {
    const features = appSpec.features || [];
    const screens = [];
    const components = [];
    const dataViews = [];
    ...
    for (const feature of features) {
        ...
        const screenComponents = ["comp-sidebar-nav"];
        const screenDataViews = [];
        const screenForms = [];
        const screenActions = [];
...
function addStates(states, screenId) {
    const stateTypes = [
...
function generateDesignBrief(appSpec, intentBrief) {
    const features = appSpec.features || [];
    const screens = [];
    ...
    const screenSpecs = screens.map(s => {
        const parts = [`### ${s.name}\n- Purpose: ${s.purpose}`];
        if (s.key_components.length > 0)
            parts.push(`- Components: ${s.key_components.join(", ")}`);
        if (s.data_views.length > 0)
            parts.push(`- Data views: ${s.data_views.join(", ")}`);
        if (s.forms.length > 0)
            parts.push(`- Forms: ${s.forms.join(", ")}`);
        if (s.actions.length > 0)
            parts.push(`- Actions: ${s.actions.join(", ")}`);
        parts.push(`- States: ${s.states.join(", ")}`);
        return parts.join("\n");
    }).join("\n\n");
    const directionSpecs = directions.map((d, i) => `**Direction ${i + 1}: ${d.name}**\n${d.description}\n- Layout: ${d.layout}\n- Colors: ${d.color_mood}\n- Density: ${d.density}\n- Typography: ${d.typography}`).join("\n\n");
