...
import { linkIncident, updatePatternFromIncident, recordFixApplication, } from "@aes/failure-memory";
/**
 ...
 */
export async function fixTrailRecorder(state) {
    ...
    const knownPatterns = (graphCtx?.failureHistory ?? []).filter((item) => item.pattern_id && item.failure_type);
    const knownFixes = (graphCtx?.fixPatterns ?? []).filter((item) => item.pattern_id && item.target_failure_patterns);
    ...
    for (const [featureId, result] of Object.entries(buildResults)) {
        ...
        for (const event of repairEvents) {
            ...
            if (link.matched_pattern) {
                ...
                cb?.onStep(`Fix [${fixId}] linked to pattern "${link.matched_pattern.name}" ` +
                ...
                if (isResolved && link.suggested_fix_pattern_id) {
                    const matchedFix = knownFixes.find((f) => f.pattern_id === link.suggested_fix_pattern_id);
                    if (matchedFix) {
                ...
                cb?.onStep(`Fix [${fixId}] recorded for "${featureId}" — no known pattern match`);
            }
        }
    }
    ...
    return {
        ...
        ...(updatedPatterns.length > 0 || updatedFixes.length > 0
            ? {
                graphContext: {
                    ...(state.graphContext ?? {}),
                    failureHistory: mergePatterns(state.graphContext?.failureHistory ?? [], updatedPatterns),
                    fixPatterns: mergeFixes(state.graphContext?.fixPatterns ?? [], updatedFixes),
                },
            }
            : {}),
    };
 ...
 */
function mergePatterns(existing, updated) {
 ...
 */
function mergeFixes(existing, updated) {
    const map = new Map(existing.map((f) => [f.pattern_id, f]));
