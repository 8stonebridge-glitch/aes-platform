 ...
 */
function cypherPriorBuilds(keywords) {
 ...
 */
function cypherSimilarFeatures(keywords) {
    const conditions = keywords
        .map((kw) => `(toLower(e.name) CONTAINS '${esc(kw)}' OR toLower(v.snapshot_description) CONTAINS '${esc(kw)}')`)
        .join(" OR ");
    return `
 ...
 */
function cypherKnownPatterns(keywords) {
    const conditions = keywords
 ...
 */
function cypherFailureHistory(keywords) {
    const conditions = keywords
 ...
 */
function cypherReusableBridges(keywords) {
    const conditions = keywords
 ...
 */
function cypherLearnedFeatures(keywords) {
    const conditions = keywords
 ...
 */
function cypherLearnedModels(keywords) {
    const conditions = keywords
 ...
 */
function cypherLearnedIntegrations(keywords) {
    const conditions = keywords
 ...
 */
function cypherLearnedPatterns(keywords) {
    const conditions = keywords
 ...
 */
function cypherLearnedFlows(keywords) {
    const conditions = keywords
 ...
 */
function cypherLearnedResearch(keywords) {
    const conditions = keywords
 ...
 */
function cypherLearnedCorrections(keywords) {
    const conditions = keywords
 ...
 */
function cypherLearnedAppContext(keywords) {
    const conditions = keywords
...
export async function graphReader(state) {
    ...
    try {
        ...
        const toRanked = (rows, label, idField) => rows.map((r) => ({
        ...
        const vectorToRanked = (results) => results.map((r) => ({
            id: r.id,
            name: r.name,
            label: r.label,
 ...
 */
function fuseAndMerge(keywordRows, vectorRows, label, idField) {
    ...
    const kwRanked = keywordRows.map((r) => ({
        id: r[idField] || r.name || "",
        name: r.name || "",
        label,
        properties: r,
    }));
    const vecRanked = vectorRows.map((r) => ({
        id: r.id,
        name: r.name,
        label: r.label,
        properties: r.properties,
