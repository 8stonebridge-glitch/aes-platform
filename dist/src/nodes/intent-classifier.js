...
function classifyAppClass(input) {
...
function classifyRisk(input, appClass) {
    const lower = input.toLowerCase();
...
function classifyPlatforms(input) {
    const lower = input.toLowerCase();
    const platforms = ["web"];
    ...
    if (PLATFORM_KEYWORDS.admin_console.some((kw) => lower.includes(kw))) {
        if (!platforms.includes("admin_console"))
            platforms.push("admin_console");
    }
    return platforms;
}
function classifyIntegrations(input) {
    const lower = input.toLowerCase();
    const integrations = [];
...
function detectAmbiguity(input, appClass) {
    const flags = [];
...
function generateClarifyingQuestions(flags, _input) {
    const questions = [];
...
function inferCoreOutcome(input, appClass) {
    const outcomes = {
...
function inferPrimaryUsers(appClass) {
    const users = {
...
export function keywordClassifyIntent(rawRequest, requestId) {
...
async function llmClassifyIntent(rawRequest, requestId) {
    const llm = getLLM();
    const structured = llm.withStructuredOutput(IntentBriefSchema);
    const result = await safeLLMCall("intent-classifier", () => structured.invoke([
        {
            role: "system",
...
export async function intentClassifier(state) {
    ...
    const hasPriorKnowledge = (graphCtx?.priorBuilds?.length || 0) > 0 ||
        (graphCtx?.similarFeatures?.length || 0) > 0;
    if (hasPriorKnowledge) {
        cb?.onStep(`Graph context: ${graphCtx.priorBuilds.length} prior builds, ${graphCtx.similarFeatures.length} similar features inform classification`);
    }
    let brief;
    let usedLLM = false;
    if (isLLMAvailable()) {
    ...
    if (hasPriorKnowledge) {
        ...
        if (priorAppClasses.some((p) => p.includes(classLower) || classLower.includes(p))) {
        ...
        const priorDescriptions = [
            ...graphCtx.priorBuilds.map((b) => b.description || ""),
            ...graphCtx.similarFeatures.map((f) => f.description || ""),
        ].join(" ").toLowerCase();
        for (const [type, keywords] of Object.entries(INTEGRATION_KEYWORDS)) {
            if (!brief.inferred_integrations.includes(type) &&
                keywords.some((kw) => priorDescriptions.includes(kw))) {
    ...
    if (brief.inferred_integrations.length > 0)
        cb?.onStep(`Integrations: ${brief.inferred_integrations.join(", ")}`);
    if (brief.ambiguity_flags.length > 0)
        cb?.onWarn(`Ambiguity flags: ${brief.ambiguity_flags.join(", ")}`);
    store.addLog(jobId, {
        gate: "gate_0",
