...
import { computeConfidence, computeTestCoverage, computeContradictionPenalty, canTransition, } from "@aes/math";
/**
 ...
 */
function runValidationRules(spec) {
    const results = [];
    // 1. ALL_FEATURES_HAVE_ACTORS
    const featuresWithoutActors = spec.features.filter((f) => !f.actor_ids || f.actor_ids.length === 0);
    results.push({
    ...
    const featuresWithoutOutcomes = spec.features.filter((f) => !f.outcome || f.outcome.trim().length === 0);
    results.push({
    ...
    const invalidWorkflowRefs = (spec.workflows || []).flatMap((w) => (w.steps || []).filter((s) => s.feature_id && !featureIds.has(s.feature_id)));
    results.push({
    ...
    const invalidPermRoles = spec.permissions.filter((p) => !roleIds.has(p.role_id));
    results.push({
    ...
    const invalidPermResources = spec.permissions.filter((p) => !validResources.has(p.resource));
    results.push({
    ...
    const undefinedDeps = (spec.dependency_graph || []).filter((e) => !featureIds.has(e.from_feature_id) || !featureIds.has(e.to_feature_id));
    results.push({
    ...
    const criticalFeatures = spec.features.filter((f) => f.priority === "critical" || f.priority === "high");
    const testedFeatures = new Set(spec.acceptance_tests.map((t) => t.feature_id));
    const untestedCritical = criticalFeatures.filter((f) => !testedFeatures.has(f.feature_id));
    results.push({
        code: GateErrorCode.G1_CRITICAL_FEATURES_NO_TESTS,
        passed: untestedCritical.length === 0,
        reason: untestedCritical.length > 0
            ? `Critical features without tests: ${untestedCritical.map((f) => f.feature_id).join(", ")}`
            : undefined,
    });
    // 8. ALL_EXTERNAL_INTEGRATIONS_DECLARE_FALLBACK_STATUS
    const integrationsWithoutFallback = (spec.integrations || []).filter((i) => i.fallback_defined === false);
    results.push({
        ...
        reason: integrationsWithoutFallback.length > 0
            ? `Integrations without fallback: ${integrationsWithoutFallback.map((i) => i.name).join(", ")}`
            : undefined,
    });
    ...
    const offlineFeatures = spec.features.filter((f) => f.offline_behavior_required);
    results.push({
    ...
    const allReferencedActors = new Set();
    for (const f of spec.features) {
        for (const actorId of f.actor_ids || []) {
            if (!EXEMPT_ACTORS.has(actorId))
                allReferencedActors.add(actorId);
        }
    }
    for (const p of spec.permissions) {
        if (!EXEMPT_ACTORS.has(p.role_id))
            allReferencedActors.add(p.role_id);
    }
    ...
    const undeclaredActors = [...allReferencedActors].filter((a) => !declaredRoleIds.has(a));
    results.push({
...
export async function specValidator(state) {
    ...
    const resolvedDeps = depGraph.filter((e) => featureIds.has(e.from_feature_id) && featureIds.has(e.to_feature_id)).length;
    const catalogMatchCount = Object.keys(state.featureBridges || {}).length;
    const testedFeatureSet = new Set(spec.acceptance_tests.map((t) => t.feature_id));
