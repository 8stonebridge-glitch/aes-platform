...
import { getTestsByCategory, SEED_REQUIREMENTS, CONTRACT_TEST_SUMMARY, } from "../contracts/opsuite-contract-tests.js";
 ...
 */
export async function validatorRunner(state) {
    ...
    for (const heuristic of heuristics) {
        ...
        try {
            ...
            const isRelevant = affectedPatterns.length === 0 || affectedPatterns.some((p) => builtFeatures.some((f) => f.toLowerCase().includes(p.toLowerCase())));
            if (!isRelevant)
                continue;
    ...
    for (const rule of preventionRules) {
        ...
        try {
            ...
            if (ruleGate !== "gate_4" && ruleGate !== "gate_5" && ruleGate !== "validation")
                continue;
            const builtFeatures = Object.keys(state.buildResults || {});
            const affectedPatterns = rule.target_failure_patterns ?? [];
            const ruleTriggered = affectedPatterns.some((p) => builtFeatures.some((f) => f.toLowerCase().includes(p.toLowerCase())));
            results.push({
    ...
    for (const featureId of Object.keys(bridges)) {
        ...
        if (!bridge?.bridge_id || !bridge?.write_scope)
            continue;
    ...
    cb?.onSuccess(shouldRunContracts
...
function shouldRunOpsuiteContracts(state) {
    const corpus = [
        state.rawRequest,
        state.intentBrief?.normalized_request,
        state.appSpec?.title,
        ...(state.appSpec?.features || []).map((f) => `${f.name} ${f.description || ""}`),
    ]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
    if (corpus.includes("opsuite"))
        return true;
 ...
 */
function determineCategories(state) {
    const builtFeatures = Object.keys(state.buildResults || {});
    ...
    const hasAuthFeature = builtFeatures.some(f => f.toLowerCase().includes("auth") || f.toLowerCase().includes("role"));
    if (hasAuthFeature) {
        categories.push("role_visibility");
        categories.push("role_isolation"); // Always run isolation with auth/role changes
    }
    const hasTaskFeature = builtFeatures.some(f => f.toLowerCase().includes("task") || f.toLowerCase().includes("workflow"));
    if (hasTaskFeature) {
        categories.push("state_machine");
    }
    // Any UI/layout/navigation change triggers role isolation tests
    const hasUIFeature = builtFeatures.some(f => f.toLowerCase().includes("ui") || f.toLowerCase().includes("layout") ||
 ...
 */
async function runTestGroup(_jobId, tests, _groupType) {
    const results = [];
