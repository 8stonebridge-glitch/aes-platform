...
export function featureText(f) {
    const parts = [f.name];
    if (f.app_name)
        parts.push(`from ${f.app_name}`);
    if (f.app_class)
        parts.push(`(${f.app_class.replace(/_/g, " ")})`);
    if (f.description)
        parts.push(`— ${f.description}`);
    if (f.complexity)
        parts.push(`[${f.complexity}]`);
    if (f.has_api)
        parts.push("with API");
    if (f.has_tests)
        parts.push("tested");
    if (f.directory)
        parts.push(`in ${f.directory}`);
    return parts.join(" ");
}
export function modelText(m) {
    const parts = [`${m.name} data model`];
    if (m.category && m.category !== "general")
        parts.push(`(${m.category.replace(/_/g, " ")})`);
    if (m.app_name)
        parts.push(`from ${m.app_name}`);
    if (m.app_class)
        parts.push(`(${m.app_class.replace(/_/g, " ")})`);
    if (m.fields_csv)
        parts.push(`fields: ${m.fields_csv.slice(0, 300)}`);
    if (m.relation_count && m.relation_count > 0)
        parts.push(`${m.relation_count} relations`);
    return parts.join(" ");
}
export function integrationText(i) {
    const parts = [i.provider || i.name];
    if (i.type)
        parts.push(`${i.type.replace(/_/g, " ")} integration`);
    if (i.app_name)
        parts.push(`used by ${i.app_name}`);
    if (i.auth_method && i.auth_method !== "unknown")
        parts.push(`auth: ${i.auth_method}`);
    return parts.join(" ");
}
export function patternText(p) {
    ...
    if (p.type)
        parts.push(`[${p.type}]`);
    if (p.app_name)
        parts.push(`from ${p.app_name}`);
    if (p.description)
        parts.push(`— ${p.description.slice(0, 250)}`);
    if (p.applicable_to)
        parts.push(`applies to: ${p.applicable_to}`);
    return parts.join(" ");
}
export function flowText(f) {
    ...
    if (f.app_name)
        parts.push(`from ${f.app_name}`);
    if (f.section)
        parts.push(`in ${f.section}`);
    if (f.step_count)
        parts.push(`(${f.step_count} steps)`);
    if (f.steps_description)
        parts.push(`— ${f.steps_description.slice(0, 250)}`);
    return parts.join(" ");
}
const INDEX_MAP = {
 ...
 */
export async function vectorSearch(queryText, nodeType, topK, neo4jRun) {
    ...
    if (!queryEmbedding)
        return [];
    try {
        const rows = await neo4jRun(`CALL db.index.vector.queryNodes($indexName, $topK, $embedding)
       YIELD node, score
       ...
       ORDER BY score DESC`, { indexName: indexInfo.index, topK, embedding: queryEmbedding });
        return rows.map((r) => ({
            id: r.id || r.name,
            name: r.name,
            label: nodeType,
 ...
 */
export async function vectorSearchAll(queryText, topK, neo4jRun) {
    const types = Object.keys(INDEX_MAP);
    const results = await Promise.all(types.map(t => vectorSearch(queryText, t, topK, neo4jRun)));
