...
// EXTRACTION LOGIC
// ═══════════════════════════════════════════════════════════════════════
export function extractDesignEvidence(input) {
    ...
    const screens = input.screens.map((s) => {
        ...
        // Build data views
        const dataViews = (s.data_views ?? []).map((dv) => ({
            view_id: makeId("view", dv.name),
            name: dv.name,
            type: dv.type,
            screen_id: screenId,
            implied_model: dv.implied_model,
            columns: (dv.columns ?? []).map((col) => ({
                name: col.name,
                type: (col.type ?? "text"),
    ...
    function addEdge(from, to, trigger, label) {
    ...
    for (const action of allActions) {
        if (action.target_screen_id) {
            addEdge(action.screen_id, action.target_screen_id, "action", action.label);
        }
    }
    const navigation = {
    ...
    // ── Layout ─────────────────────────────────────────────────────────
    const layout = {
        pattern: (input.layout?.pattern ?? "custom"),
        responsive_notes: input.layout?.responsive_notes ?? [],
        sidebar: input.layout?.sidebar
            ? {
                position: (input.layout.sidebar.position ?? "left"),
    ...
    for (const screen of screens) {
        if (screen.component_ids.length === 0) {
            ...
            deductions.push(`Screen "${screen.name}" has no components (-0.05)`);
        }
        if (screen.data_view_ids.length === 0 &&
            screen.form_ids.length === 0 &&
            screen.action_ids.length === 0) {
            confidenceScore -= 0.05;
            deductions.push(`Screen "${screen.name}" has no views, forms, or actions (-0.05)`);
        }
    }
    for (const action of allActions) {
        if (action.type === "custom" && !action.implied_operation) {
            confidenceScore -= 0.02;
            deductions.push(`Action "${action.label}" has no implied operation (-0.02)`);
        }
    }
 ...
 */
export async function persistDesignEvidence(evidence) {
    const neo = getNeo4jService();
    const ok = await neo.connect();
    if (!ok) {
        console.warn("[design-extract] Neo4j unavailable — skipping persistence");
        return;
    }
    console.log(`[design-extract] Persisting evidence ${evidence.evidence_id} to Neo4j...`);
    // ── Evidence root node ─────────────────────────────────────────────
    await neo.runCypher(`MERGE (e:DesignEvidence {evidence_id: $eid})
    ...
    for (const screen of evidence.screens) {
        await neo.runCypher(`MERGE (s:DesignScreen {screen_id: $sid})
       SET s.name       = $name,
           s.purpose    = $purpose,
       ...
       MERGE (e)-[:HAS_SCREEN]->(s)`, {
        ...
        for (const compId of screen.component_ids) {
            const comp = evidence.components.find((c) => c.component_id === compId);
            if (!comp)
                continue;
            await neo.runCypher(`MERGE (c:DesignComponent {component_id: $cid})
         SET c.name         = $name,
             c.category     = $category,
             c.purpose      = $purpose,
        ...
        // ── Data views ───────────────────────────────────────────────────
        const screenDataViews = evidence.data_views.filter((dv) => dv.screen_id === screen.screen_id);
        for (const dv of screenDataViews) {
            await neo.runCypher(`MERGE (v:DesignDataView {view_id: $vid})
         SET v.name          = $name,
             v.type          = $type,
        ...
        // ── Forms ────────────────────────────────────────────────────────
        const screenForms = evidence.forms.filter((f) => f.screen_id === screen.screen_id);
        for (const form of screenForms) {
            await neo.runCypher(`MERGE (f:DesignForm {form_id: $fid})
         SET f.name           = $name,
        ...
        // ── Actions ──────────────────────────────────────────────────────
        const screenActions = evidence.actions.filter((a) => a.screen_id === screen.screen_id);
        for (const action of screenActions) {
            await neo.runCypher(`MERGE (a:DesignAction {action_id: $aid})
         SET a.label             = $label,
             a.type              = $type,
        ...
        // ── States ───────────────────────────────────────────────────────
        const screenStates = evidence.states.filter((st) => st.screen_id === screen.screen_id);
        for (const state of screenStates) {
            await neo.runCypher(`MERGE (st:DesignState {state_id: $stid})
         SET st.type        = $type,
    ...
    for (const edge of evidence.navigation.edges) {
        await neo.runCypher(`MATCH (from:DesignScreen {screen_id: $fromId})
    ...
    console.log(`[design-extract] Persisted: ${evidence.screens.length} screens, ` +
 ...
 */
function normalizePaperInput(raw) {
    const data = raw;
    const treeSummary = (data.tree_summary ?? data);
    const name = treeSummary.name ??
        treeSummary.fileName ??
        "Paper Design";
    const artboards = (treeSummary.artboards ??
        treeSummary.children ??
        []);
    const screens = artboards.map((ab) => {
        const abName = ab.name ?? "Untitled Screen";
        const children = (ab.children ?? []);
        const componentNames = children
            .filter((c) => c.type === "component" ||
            c.type === "instance")
            .map((c) => c.name ?? "Unknown Component");
        return {
            name: abName,
            purpose: `Screen from Paper artboard "${abName}"`,
            artboard_ref: ab.id ?? undefined,
            dimensions: ab.width
                ? {
                    width: ab.width,
                    height: ab.height,
    ...
    return {
        name,
        source_type: "paper",
        source_ref: treeSummary.fileId ?? "paper-mcp",
        screens,
    };
}
// ═══════════════════════════════════════════════════════════════════════
...
Options:
  ...
  --help            Show this help`);
    ...
    if (mode === "json" || mode === "paper") {
        ...
        console.log("Interactive mode: paste a JSON design description and press Ctrl+D when done.");
        const chunks = [];
        ...
        try {
            designInput = JSON.parse(stdinContent);
        }
        catch (err) {
    ...
    console.log(`[design-extract] Extracted: ${evidence.screens.length} screens, ` +
        `${evidence.components.length} components, ` +
        `${evidence.data_views.length} data views, ` +
        `${evidence.forms.length} forms, ` +
        `${evidence.actions.length} actions, ` +
    ...
    if (evidence.extraction_meta.warnings.length > 0) {
        console.log(`[design-extract] Warnings (${evidence.extraction_meta.warnings.length}):`);
        for (const w of evidence.extraction_meta.warnings) {
            console.log(`  - [${w.type}] ${w.message}`);
        }
    }
    console.log(`[design-extract] Confidence: ${evidence.extraction_meta.confidence}`);
    // ── Output ─────────────────────────────────────────────────────────
    ...
    if (pushJobId) {
        ...
        try {
            ...
            if (resp.ok) {
                console.log(`[design-extract] Pipeline notified — ${result.screens} screens, ${result.components} components received. Pipeline will resume.`);
            }
            else {
                console.error(`[design-extract] API error: ${result.error}`);
            }
        }
        catch (err) {
...
const isDirectRun = process.argv[1]?.endsWith("design-extract.ts") ||
