...
// MAIN NORMALIZER
// ═══════════════════════════════════════════════════════════════
export function normalizeDesignEvidence(raw) {
    const evidenceId = raw.evidence_id ?? `de-${randomUUID().slice(0, 8)}`;
    ...
    const screens = (raw.screens ?? []).map((rs) => {
        ...
        const compIds = new Set(rs.component_ids ??
    ...
    for (const rawDv of raw.data_views ?? []) {
        const id = rawDv.view_id ?? rawDv.data_view_id ?? rawDv.id ?? makeId("view", rawDv.name);
    ...
    for (const rawState of raw.states ?? []) {
        const id = rawState.state_id ?? rawState.id ?? makeId("state", `${rawState.type}`);
    ...
    const warnings = (meta.warnings ?? []).map((w) => ({
        type: (w.type ?? "ambiguous_component"),
        message: w.message ?? "",
        ref: w.ref,
    }));
...
// ═══════════════════════════════════════════════════════════════
function normalizeComponent(rc, defaultScreenId) {
    const id = rc.component_id ?? rc.id ?? makeId("comp", rc.name);
    ...
    return {
        ...
        category,
        purpose: rc.purpose ?? "",
        screen_ids: rc.screen_ids && rc.screen_ids.length > 0
            ? rc.screen_ids
            : defaultScreenId
                ? [defaultScreenId]
                : [],
...
function normalizeDataView(rv, defaultScreenId) {
    const id = rv.view_id ?? rv.data_view_id ?? rv.id ?? makeId("view", rv.name);
    ...
    const columns = (rv.columns ?? []).map((c) => ({
        name: c.name,
    ...
    const capabilities = (rv.capabilities ?? []).filter((c) => VALID_CAPABILITIES.has(c));
    return {
        view_id: id,
        name: rv.name,
        type: viewType,
...
function normalizeForm(rf, defaultScreenId) {
    const id = rf.form_id ?? rf.id ?? makeId("form", rf.name);
    const fields = (rf.fields ?? []).map((f) => ({
...
function normalizeAction(ra, defaultScreenId) {
    ...
    const isDestructive = ra.is_destructive ?? DESTRUCTIVE_ACTION_TYPES.has(actionType);
...
function normalizeState(rs, defaultScreenId) {
    const id = rs.state_id ?? rs.id ?? makeId("state", `${rs.type}`);
...
function normalizeLayout(raw) {
    ...
    return {
        ...
        sidebar: raw.sidebar
            ? {
                position: (raw.sidebar.position === "right" ? "right" : "left"),
