...
import { vectorSearch, isEmbeddingAvailable, } from "../services/embedding-service.js";
// ═══════════════════════════════════════════════════════════════════════
// WEIGHTS
// ═══════════════════════════════════════════════════════════════════════
 ...
 */
async function featureSignal(query, run) {
    const useVector = isEmbeddingAvailable();
    if (useVector) {
        const results = await vectorSearch(`${query.name}: ${query.description}`, "LearnedFeature", TOP_K_VECTOR, run);
    ...
    if (keywords.length === 0)
        return new Map();
    const conditions = keywords.map((_, i) => `(toLower(f.name) CONTAINS $kw${i} OR toLower(f.description) CONTAINS $kw${i})`);
    const params = {};
    keywords.forEach((kw, i) => { params[`kw${i}`] = kw; });
    const rows = await run(`MATCH (a:LearnedApp)-[:HAS_FEATURE]->(f:LearnedFeature)
     WHERE ${conditions.join(" OR ")}
     RETURN a.name AS app, f.name AS feature,
            ${keywords.map((_, i) => `CASE WHEN toLower(f.name) CONTAINS $kw${i} OR toLower(f.description) CONTAINS $kw${i} THEN 1 ELSE 0 END`).join(" + ")} AS hits`, params);
    const groups = new Map();
    for (const r of rows) {
        const app = r.app;
        if (!groups.has(app))
            groups.set(app, { totalHits: 0, count: 0, matched: [] });
        const g = groups.get(app);
        g.totalHits += r.hits || 1;
        g.count += 1;
        g.matched.push(r.feature);
    }
    const out = new Map();
    for (const [app, g] of groups) {
 ...
 */
async function modelSignal(query, run) {
    if (query.required_models && query.required_models.length > 0) {
        const lowerModels = query.required_models.map((m) => m.toLowerCase());
        const rows = await run(`MATCH (a:LearnedApp)-[:HAS_DATA_MODEL]->(m:LearnedDataModel)
       WHERE toLower(m.name) IN $models
       RETURN a.name AS app, collect(m.name) AS matched`, { models: lowerModels });
        const out = new Map();
        for (const r of rows) {
    ...
    if (isEmbeddingAvailable()) {
        const results = await vectorSearch(query.description, "LearnedDataModel", TOP_K_VECTOR, run);
        const grouped = groupByApp(results);
        const out = new Map();
        for (const [app, g] of grouped) {
            out.set(app, { score: g.avgScore, matched: g.matched });
 ...
 */
async function integrationSignal(query, run) {
    if (query.required_integrations && query.required_integrations.length > 0) {
        ...
        const rows = await run(`MATCH (a:LearnedApp)-[:USES_INTEGRATION]->(i:LearnedIntegration)
       WHERE toLower(i.provider) IN $integrations OR toLower(i.name) IN $integrations
       RETURN a.name AS app, collect(COALESCE(i.provider, i.name)) AS matched`, { integrations: lowerIntegrations });
        const out = new Map();
        for (const r of rows) {
            const matched = r.matched || [];
            out.set(r.app, {
    ...
    if (isEmbeddingAvailable()) {
        const results = await vectorSearch(query.description, "LearnedIntegration", TOP_K_VECTOR, run);
        const grouped = groupByApp(results);
        const out = new Map();
        for (const [app, g] of grouped) {
            out.set(app, { score: g.avgScore, matched: g.matched });
 ...
 */
async function patternSignal(query, run) {
    if (query.required_patterns && query.required_patterns.length > 0) {
        ...
        const rows = await run(`MATCH (a:LearnedApp)-[:HAS_PATTERN]->(p:LearnedPattern)
       WHERE toLower(p.type) IN $patterns OR toLower(p.name) IN $patterns
       RETURN a.name AS app, collect(p.name) AS matched`, { patterns: lowerPatterns });
        const out = new Map();
        for (const r of rows) {
            const matched = r.matched || [];
            out.set(r.app, {
    ...
    // Fallback to vector search
    if (isEmbeddingAvailable()) {
        const results = await vectorSearch(query.description, "LearnedPattern", TOP_K_VECTOR, run);
        const grouped = groupByApp(results);
        const out = new Map();
        for (const [app, g] of grouped) {
            out.set(app, { score: g.avgScore, matched: g.matched });
 ...
 */
async function loadAppClasses(run) {
    const rows = await run(`MATCH (a:LearnedApp)
     RETURN a.name AS name, a.app_class AS app_class`);
    const out = new Map();
    for (const r of rows) {
        out.set(r.name, r.app_class || "unknown");
    }
    return out;
}
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
async function generateSuggestions(appName, matchedFeatures, matchedModels, matchedPatterns, run) {
    const suggestions = [];
    // Feature suggestions with descriptions
    if (matchedFeatures.length > 0) {
        const featureDetails = await run(`MATCH (a:LearnedApp {name: $app})-[:HAS_FEATURE]->(f:LearnedFeature)
       WHERE f.name IN $features
       RETURN f.name AS name, f.description AS description`, { app: appName, features: matchedFeatures });
    ...
    if (matchedModels.length > 0) {
        const modelDetails = await run(`MATCH (a:LearnedApp {name: $app})-[:HAS_DATA_MODEL]->(m:LearnedDataModel)
       WHERE m.name IN $models
       RETURN m.name AS name, m.field_count AS fields`, { app: appName, models: matchedModels });
 ...
 */
export async function findDonors(query) {
    ...
    const run = (cypher, params) => neo4j.runCypher(cypher, params);
    // Run all signals in parallel
    const [featureSig, modelSig, integrationSig, patternSig, appClasses] = await Promise.all([
    ...
    for (const app of allApps) {
        ...
        const classAffinity = query.app_class_hint &&
            appClasses.get(app)?.toLowerCase() === query.app_class_hint.toLowerCase()
            ? 1.0
            : 0.0;
        const overall = WEIGHT_FEATURE * fs.score +
    ...
    await Promise.all(top.map(async (donor) => {
        if (donor.overall_score >= SUGGESTION_THRESHOLD) {
            donor.reuse_suggestions = await generateSuggestions(donor.app_name, donor.matched_features, donor.matched_models, donor.matched_patterns, run);
        }
    }));
    return top;
}
...
function printResults(query, results) {
    ...
    // Table header
    console.log("  ┌─────┬──────────────────────────┬──────────┬────────────────────────────────┐");
    console.log("  │ #   │ App                      │ Score    │ Signals (F/M/I/P)              │");
    console.log("  ├─────┼──────────────────────────┼──────────┼────────────────────────────────┤");
    for (let i = 0; i < results.length; i++) {
        ...
        console.log(`  │ ${rank}  │ ${name} │ ${score}  │ ${signals.padEnd(30)} │`);
    }
    console.log("  └─────┴──────────────────────────┴──────────┴────────────────────────────────┘");
    console.log();
...
async function main() {
    ...
    if (args.length === 0) {
        ...
        console.log('  npx tsx src/tools/donor-match.ts --json \'{"name":"auth","description":"SSO with SAML"}\'');
        process.exit(1);
    }
    let query;
    if (args[0] === "--json") {
...
const isMain = process.argv[1]?.endsWith("donor-match.ts") ||
