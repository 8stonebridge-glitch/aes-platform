...
async function main() {
    ...
    for (const [name, correctClass] of Object.entries(FIXES)) {
        const result = await neo4j.runCypher(`MATCH (a:LearnedApp {name: $name})
       SET a.app_class = $cls
       RETURN a.name AS name, a.app_class AS cls`, { name, cls: correctClass });
        if (result.length > 0) {
  ...
  `);
    ...
    await neo4j.runCypher(`MATCH (a:LearnedApp {name: "root"})
     WHERE a.source_url CONTAINS "medusa" OR a.source_path CONTAINS "medusa"
     SET a.name = "medusa"
     RETURN a.name`);
    // Rename "n8n-monorepo" to "n8n"
    await neo4j.runCypher(`MATCH (a:LearnedApp {name: "n8n-monorepo"})
     SET a.name = "n8n"
     RETURN a.name`);
    // Rename "@logto/root" to "logto"
    await neo4j.runCypher(`MATCH (a:LearnedApp {name: "@logto/root"})
     SET a.name = "logto"
     RETURN a.name`);
