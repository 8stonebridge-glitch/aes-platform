...
let neo4j;
async function q(cypher) { try {
    return await neo4j.runCypher(cypher);
}
catch {
    return [];
} }
function val(v) { return v && typeof v === "object" && "low" in v ? v.low : v; }
...
async function testSection(name, perplexityItems, cypher, matchField) {
...
async function main() {
    ...
    console.log("═══════════════════════════════════════════════════════════\n");
    // 1. FEATURES
    const featResult = await testSection("FEATURES", PERPLEXITY.features, `MATCH (a:LearnedApp)-[:HAS_FEATURE]->(f:LearnedFeature)
     WHERE a.app_class CONTAINS 'scheduling'
     RETURN f.name AS name ORDER BY f.file_count DESC LIMIT 30`, "name");
    results.push(featResult);
    // Also check all features across all apps
    const featResult2 = await testSection("FEATURES (all apps)", PERPLEXITY.features, `MATCH (f:LearnedFeature) WITH DISTINCT f.name AS name, f.file_count AS fc ORDER BY fc DESC LIMIT 100 RETURN name`, "name");
    results.push(featResult2);
    // 2. DATA MODELS
    const modelNames = PERPLEXITY.models.map(m => m.name);
    const modelResult = await testSection("DATA MODELS", modelNames, `MATCH (m:LearnedDataModel) WITH DISTINCT m.name AS name, m.field_count AS fc ORDER BY fc DESC RETURN name`, "name");
    results.push(modelResult);
    // 3. INTEGRATIONS
    const intNames = PERPLEXITY.integrations.map(i => i.name);
    const intResult = await testSection("INTEGRATIONS", intNames, `MATCH (i:LearnedIntegration)
     RETURN DISTINCT i.provider AS name ORDER BY i.provider`, "name");
    results.push(intResult);
    // 4. AUTH PATTERNS
    const authResult = await testSection("AUTH PATTERNS", PERPLEXITY.auth, `MATCH (p:LearnedPattern) WHERE p.type = 'auth'
     RETURN p.name AS name ORDER BY p.name`, "name");
    results.push(authResult);
    // 5. TECH STACK
    const techResult = await testSection("TECH STACK", PERPLEXITY.techStack, `MATCH (a:LearnedApp)
     RETURN a.framework + ' ' + a.database + ' ' + a.orm + ' ' + a.styling + ' ' + a.api_style AS name
     ORDER BY a.total_files DESC`, "name");
    results.push(techResult);
    // 6. USER FLOWS
    const flowResult = await testSection("USER FLOWS", PERPLEXITY.userFlows, `MATCH (uf:LearnedUserFlow)
     RETURN uf.name + ' ' + uf.steps AS name ORDER BY uf.name`, "name");
    results.push(flowResult);
    // 7. UI PATTERNS
    const uiResult = await testSection("UI PATTERNS", PERPLEXITY.uiPatterns, `MATCH (c:LearnedComponentGroup)
     ...
     UNION
     MATCH (sp:LearnedStatePattern) RETURN sp.component + ' ' + sp.type AS name`, "name");
