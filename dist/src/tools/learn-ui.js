...
// ─── Scanners ───────────────────────────────────────────────────────
function readSafe(filePath, maxLines = 200) {
    try {
...
function scanComponents(rootDir) {
    const components = [];
    ...
    for (const uiDir of uiDirs) {
        ...
        for (const file of tsxFiles) {
            ...
            if (name.startsWith("_") || name === "index" || name.includes(".test") || name.includes(".stories"))
                continue;
...
function categorizeComponent(filePath, name) {
    const lower = (filePath + name).toLowerCase();
    if (lower.includes("form") || lower.includes("input") || lower.includes("select") || lower.includes("checkbox") || lower.includes("switch") || lower.includes("radio"))
        return "form";
    if (lower.includes("dialog") || lower.includes("modal") || lower.includes("sheet") || lower.includes("popover") || lower.includes("overlay"))
        return "overlay";
    if (lower.includes("nav") || lower.includes("sidebar") || lower.includes("menu") || lower.includes("breadcrumb") || lower.includes("tab"))
        return "navigation";
    if (lower.includes("button") || lower.includes("badge") || lower.includes("icon") || lower.includes("avatar") || lower.includes("logo"))
        return "element";
    if (lower.includes("table") || lower.includes("list") || lower.includes("card") || lower.includes("grid"))
        return "data_display";
    if (lower.includes("skeleton") || lower.includes("loading") || lower.includes("spinner") || lower.includes("progress"))
        return "loading";
    if (lower.includes("error") || lower.includes("empty") || lower.includes("alert"))
        return "feedback";
    if (lower.includes("layout") || lower.includes("shell") || lower.includes("container") || lower.includes("section"))
        return "layout";
    if (lower.includes("toast") || lower.includes("banner") || lower.includes("notification"))
        return "notification";
    if (lower.includes("editor") || lower.includes("rich-text") || lower.includes("markdown"))
        return "editor";
    if (lower.includes("calendar") || lower.includes("date") || lower.includes("time"))
        return "datetime";
...
function scanPages(rootDir) {
    ...
    for (const pageDir of pageDirs) {
        ...
        if (!fs.existsSync(fullDir))
            continue;
        const scanRoutes = (dir, route, section) => {
            try {
                for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
                    if (entry.name.startsWith(".") || entry.name === "node_modules" || entry.name === "_components")
                        continue;
                    const full = path.join(dir, entry.name);
                    if (entry.isDirectory()) {
...
function scanNavigation(rootDir) {
    const navItems = [];
...
function scanDesignTokens(rootDir) {
    const tokens = {
        colors: {},
        fonts: [],
        spacing: [],
    ...
    for (const twc of twConfigs) {
        ...
        if (fs.existsSync(full)) {
            ...
            for (const m of fontMatches) {
                ...
                if (fonts)
                    tokens.fonts.push(...fonts.map(f => f.replace(/["']/g, "")));
            }
            break;
        }
    }
    ...
    let rootPkg2 = {};
    try {
        rootPkg2 = JSON.parse(fs.readFileSync(path.join(rootDir, "package.json"), "utf-8"));
    }
    catch { }
    ...
    for (const pf of pkgFiles.slice(0, 20)) {
        let pkg = {};
        try {
            pkg = JSON.parse(fs.readFileSync(pf, "utf-8"));
        }
        catch { }
        if (pkg.dependencies)
            nestedDeps.push(...Object.keys(pkg.dependencies));
    }
    const allAllDeps = [...new Set([...allDeps, ...nestedDeps])];
    if (allAllDeps.some(d => d.includes("@radix-ui")))
        tokens.componentLibrary = "Radix UI";
    if (allAllDeps.some(d => d.includes("@base-ui")))
        tokens.componentLibrary += " + Base UI";
    if (allAllDeps.some(d => d.includes("@chakra-ui")))
        tokens.componentLibrary = "Chakra UI";
    if (allAllDeps.some(d => d.includes("@mui")))
        tokens.componentLibrary = "Material UI";
    if (allAllDeps.some(d => d.includes("@mantine")))
        tokens.componentLibrary = "Mantine";
    if (allAllDeps.some(d => d.includes("shadcn") || d.includes("@shadcn")))
        tokens.componentLibrary += " (Shadcn)";
    if (allAllDeps.some(d => d.includes("lucide")))
        tokens.iconLibrary = "Lucide";
    if (allAllDeps.some(d => d.includes("heroicons")))
        tokens.iconLibrary = "Heroicons";
    if (allAllDeps.some(d => d.includes("@phosphor")))
        tokens.iconLibrary = "Phosphor";
    if (allAllDeps.some(d => d.includes("react-icons")))
        tokens.iconLibrary = "React Icons";
    return tokens;
}
function scanLayouts(rootDir) {
    const layouts = [];
    // Find layout files
    ...
    for (const file of [...layoutFiles, ...shellFiles].slice(0, 20)) {
        ...
        if (content.includes("Sidebar") || content.includes("SideBar"))
            structure.push("sidebar");
        if (content.includes("TopNav") || content.includes("Header") || content.includes("Navbar"))
            structure.push("header");
        if (content.includes("Footer"))
            structure.push("footer");
        if (content.includes("Banner"))
            structure.push("banner");
        if (content.includes("Modal") || content.includes("Dialog"))
            structure.push("modal_system");
        if (content.includes("Toast") || content.includes("Sonner"))
            structure.push("toast_system");
        if (content.includes("KBar") || content.includes("CommandPalette") || content.includes("cmdk"))
            structure.push("command_palette");
        layouts.push({
            name: `${name} (${relPath})`,
            type,
            description,
...
function scanUserFlows(rootDir) {
    const flows = [];
    // Onboarding
...
function scanFormPatterns(rootDir) {
    const patterns = [];
    const formDirs = [
...
function scanStatePatterns(rootDir) {
    const patterns = [];
...
// ─── Main Analysis ──────────────────────────────────────────────────
function analyzeUI(rootDir) {
    let rootPkg = {};
    try {
        rootPkg = JSON.parse(fs.readFileSync(path.join(rootDir, "package.json"), "utf-8"));
    }
    catch { }
...
async function writeUIToNeo4j(analysis) {
    ...
    let written = 0;
    let failed = 0;
    async function safeWrite(cypher, label) {
        try {
    ...
    for (const comp of analysis.components) {
        if (!componentsByCategory.has(comp.category))
            componentsByCategory.set(comp.category, []);
