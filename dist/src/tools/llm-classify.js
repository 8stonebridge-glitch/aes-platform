...
async function classifyApp(client, app) {
    ...
    console.warn(`  [warn] Unrecognized classification "${raw}" for ${app.name}, skipping`);
    return null;
}
// ─── Neo4j Update ────────────────────────────────────────────────────
async function updateAppClass(name, appClass) {
    const neo4j = getNeo4jService();
    await neo4j.runCypher(`MATCH (a:LearnedApp {name: $name})
     SET a.app_class = $cls, a.classified_by = "llm"
     RETURN a.name`, { name, cls: appClass });
}
// ─── Update-Scanner Output ───────────────────────────────────────────
function emitScannerSnippet(results) {
...
async function main() {
    ...
    for (const app of apps) {
        ...
        console.log(`${category}${marker}${app.currentClass ? ` (was: ${app.currentClass})` : ""}`);
        results.push({
            name: app.name,
