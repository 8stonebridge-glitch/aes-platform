 ...
 */
export function extractKnowledge(run) {
 ...
 */
export async function writeExtractionToGraph(extraction, neo4jRun) {
    let nodesCreated = 0;
    let relsCreated = 0;
...
async function main() {
    ...
    if (!inputPath) {
        ...
        const extraction = extractKnowledge(mockRun);
    ...
    const run = JSON.parse(readFileSync(inputPath, "utf-8"));
