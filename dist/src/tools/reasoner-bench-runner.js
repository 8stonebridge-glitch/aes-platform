...
const P = process.env.AES_REASONER_PARAMS
...
async function buildSynonymClusters(keywords) {
    ...
    `);
        const all = coTerms.map((r) => r.term).filter(Boolean);
...
function flattenSynonyms(clusters) {
    ...
    for (const syns of Array.from(clusters.values())) {
        for (const s of syns) {
            if (s.length >= P.synonymMinLength)
                all.add(s);
        }
    }
...
// ═══════════════════════════════════════════════════════════════════════
async function findSeedNodes(request, domains, expandedKws) {
    const seeds = [];
    ...
    for (const domain of domains) {
      ...
      `);
        ...
        `);
                for (const r of apps)
                    addSeed({ id: r.id, label: "LearnedApp", name: r.name, properties: { app_class: r.app_class } });
            }
        }
    }
...
async function discoverEdges(node) {
    ...
    if (node.label === "LearnedApp") {
        ...
        for (const r of features)
            edges.push({ type: "HAS_FEATURE", targetNode: { id: r.id, label: "LearnedFeature", name: r.name, properties: { complexity: r.complexity } }, score: 0, reason: "" });
        for (const r of models)
            edges.push({ type: "HAS_DATA_MODEL", targetNode: { id: r.name, label: "LearnedDataModel", name: r.name, properties: { category: r.category } }, score: 0, reason: "" });
        for (const r of integrations)
            edges.push({ type: "HAS_INTEGRATION", targetNode: { id: r.name, label: "LearnedIntegration", name: r.name, properties: { type: r.type } }, score: 0, reason: "" });
        for (const r of patterns)
            edges.push({ type: "USES_PATTERN", targetNode: { id: r.name, label: "LearnedPattern", name: r.name, properties: { type: r.type } }, score: 0, reason: "" });
        for (const r of flows)
            edges.push({ type: "HAS_USER_FLOW", targetNode: { id: r.name, label: "LearnedUserFlow", name: r.name, properties: {} }, score: 0, reason: "" });
    }
    if (node.label === "LearnedFeature") {
        ...
        if (app.length > 0)
            edges.push({ type: "BELONGS_TO_APP", targetNode: { id: app[0].id, label: "LearnedApp", name: app[0].name, properties: {} }, score: 0, reason: "" });
    }
    ...
    if (node.label === "LearnedPattern") {
        ...
        for (const r of apps)
            edges.push({ type: "USED_BY_APP", targetNode: { id: r.id, label: "LearnedApp", name: r.name, properties: {} }, score: 0, reason: "" });
    }
...
function scoreEdges(edges, allKeywords, hungerBonus) {
    for (const edge of edges) {
        ...
        if (target.label === "LearnedDataModel")
            score += P.modelStructuralBonus;
        if (target.label === "LearnedPattern")
            score += P.patternStructuralBonus;
        if (target.label === "LearnedUserFlow")
            score += P.flowStructuralBonus;
        if (target.properties.complexity === "complex")
            score += P.complexityBonus;
...
// ═══════════════════════════════════════════════════════════════════════
async function beamSearch(request, seedNodes, expandedKeywords) {
    const discoveredKnowledge = new Map([
        ["features", new Set()], ["models", new Set()],
        ["integrations", new Set()], ["patterns", new Set()],
        ["flows", new Set()], ["apps", new Set()],
    ]);
    const tracedPaths = [];
    ...
    for (let hop = 0; hop < P.maxHops; hop++) {
        ...
        if (cats.get("features").size < P.hungerFeatures)
            hungerBonus.set("LearnedFeature", P.hungerBonusFeature);
        if (cats.get("models").size < P.hungerModels)
            hungerBonus.set("LearnedDataModel", P.hungerBonusModel);
        if (cats.get("integrations").size < P.hungerIntegrations)
            hungerBonus.set("LearnedIntegration", P.hungerBonusIntegration);
        if (cats.get("patterns").size < P.hungerPatterns)
            hungerBonus.set("LearnedPattern", P.hungerBonusPattern);
        if (cats.get("flows").size < P.hungerFlows)
            hungerBonus.set("LearnedUserFlow", P.hungerBonusFlow);
        if (cats.get("apps").size < P.hungerApps)
            hungerBonus.set("LearnedApp", P.hungerBonusApp);
        for (const node of currentBeam) {
            ...
            switch (node.label) {
                case "LearnedApp":
                    cats.get("apps").add(node.name);
                    break;
                case "LearnedFeature":
                    cats.get("features").add(node.name);
                    break;
                case "LearnedDataModel":
                    cats.get("models").add(node.name);
                    break;
                case "LearnedIntegration":
                    cats.get("integrations").add(node.name);
                    break;
                case "LearnedPattern":
                    cats.get("patterns").add(node.name);
                    break;
                case "LearnedUserFlow":
                    cats.get("flows").add(node.name);
                    break;
            }
            ...
            for (const e of relevant) {
                if (!bestEdges.includes(e) && bestEdges.length < P.beamWidth)
                    bestEdges.push(e);
            }
...
async function main() {
    ...
    if (discoveredKnowledge.get("flows").size < 3 && discoveredApps.length > 0) {
        for (const appName of discoveredApps.slice(0, 3)) {
            ...
            for (const r of flows)
                discoveredKnowledge.get("flows").add(r.name);
        }
    }
    if (discoveredKnowledge.get("integrations").size < 3 && discoveredApps.length > 0) {
        for (const appName of discoveredApps.slice(0, 3)) {
            ...
            for (const r of integs)
                discoveredKnowledge.get("integrations").add(r.name);
        }
    }
    if (discoveredKnowledge.get("patterns").size < 3 && discoveredApps.length > 0) {
        for (const appName of discoveredApps.slice(0, 3)) {
            ...
            for (const r of pats)
                discoveredKnowledge.get("patterns").add(r.name);
        }
    }
    // Concept confidence
    const conceptScores = [];
    for (const domain of domains) {
        ...
        let conf = "GAP";
        if (nodeTypesHit >= 3 && totalHits >= 5)
            conf = "HIGH";
        else if (nodeTypesHit >= 2 && totalHits >= 3)
            conf = "MEDIUM";
        else if (totalHits >= 1)
            conf = "LOW";
