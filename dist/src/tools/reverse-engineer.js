...
function readFileSafe(filePath, maxLines = 500) {
    try {
...
// ─── Analyzers ──────────────────────────────────────────────────────
function analyzeTechStack(rootDir, rootPkg) {
    const allDeps = {
    ...
    const detect = (candidates) => {
        for (const [dep, label] of candidates) {
            if (allDepNames.some((d) => d.includes(dep)))
                return label;
        }
        return "unknown";
    };
...
function analyzeFeatures(rootDir) {
    const features = [];
...
function inferFeatureDescription(name) {
    const descriptions = {
...
function analyzePrismaModels(rootDir) {
    const models = [];
    // Find schema.prisma
    const schemaPaths = [
...
function analyzeIntegrations(rootDir) {
    const integrations = [];
    ...
    for (const appDir of appStoreDirs) {
        ...
        try {
            ...
            for (const entry of entries) {
                ...
                if (config.includes("video") || config.includes("conferencing"))
                    type = "video_conferencing";
                else if (config.includes("calendar"))
                    type = "calendar";
                else if (config.includes("payment"))
                    type = "payment";
                else if (config.includes("crm") || config.includes("CRM"))
                    type = "crm";
                else if (config.includes("messaging"))
                    type = "messaging";
                else if (config.includes("analytics"))
                    type = "analytics";
                else if (config.includes("automation"))
                    type = "automation";
                else
                    type = category;
                integrations.push({
...
function categorizeIntegration(name) {
    const lower = name.toLowerCase();
    if (lower.includes("video") || lower.includes("zoom") || lower.includes("meet") || lower.includes("teams") || lower.includes("jitsi") || lower.includes("daily"))
        return "video_conferencing";
    if (lower.includes("calendar") || lower.includes("ical") || lower.includes("caldav"))
        return "calendar";
    if (lower.includes("payment") || lower.includes("stripe") || lower.includes("paypal") || lower.includes("btcpay"))
        return "payment";
    if (lower.includes("crm") || lower.includes("hubspot") || lower.includes("salesforce") || lower.includes("pipedrive"))
        return "crm";
    if (lower.includes("analytics") || lower.includes("ga4") || lower.includes("posthog") || lower.includes("plausible"))
        return "analytics";
    if (lower.includes("zapier") || lower.includes("make") || lower.includes("n8n"))
        return "automation";
    if (lower.includes("slack") || lower.includes("discord") || lower.includes("telegram") || lower.includes("whatsapp"))
        return "messaging";
    if (lower.includes("email") || lower.includes("sendgrid") || lower.includes("resend"))
        return "email";
    return "other";
}
function analyzeApiRoutes(rootDir) {
    const routes = [];
                ...
                for (const entry of entries) {
                    ...
                    if (entry.isDirectory()) {
                        ...
                        if (content.includes("GET") || content.includes("get"))
                            methods.push("GET");
                        if (content.includes("POST") || content.includes("post") || content.includes("create") || content.includes("Create"))
                            methods.push("POST");
                        if (content.includes("PUT") || content.includes("put") || content.includes("PATCH") || content.includes("patch") || content.includes("update") || content.includes("Update"))
                            methods.push("PUT");
                        if (content.includes("DELETE") || content.includes("delete") || content.includes("remove") || content.includes("Remove"))
                            methods.push("DELETE");
                        if (methods.length === 0)
...
function analyzePatterns(rootDir, techStack, features) {
    const patterns = [];
...
function classifyApp(features, integrations, dataModels) {
    const featureNames = features.map((f) => f.name.toLowerCase()).join(" ");
    const integrationTypes = integrations.map((i) => i.type).join(" ");
    if (featureNames.includes("booking") || featureNames.includes("scheduling") || featureNames.includes("calendar"))
        return "scheduling_platform";
    if (featureNames.includes("payment") && featureNames.includes("wallet"))
        return "fintech_wallet";
    if (featureNames.includes("marketplace") || featureNames.includes("seller") || featureNames.includes("buyer"))
        return "marketplace";
    if (featureNames.includes("compliance") || featureNames.includes("audit") || featureNames.includes("case"))
        return "compliance_case_management";
    if (featureNames.includes("property") || featureNames.includes("tenant") || featureNames.includes("rental"))
        return "property_management_system";
    if (featureNames.includes("logistics") || featureNames.includes("fleet") || featureNames.includes("delivery"))
        return "logistics_operations_system";
    if (featureNames.includes("approval") || featureNames.includes("workflow"))
        return "workflow_approval_system";
    if (featureNames.includes("dashboard") || featureNames.includes("admin") || featureNames.includes("ops"))
        return "internal_ops_tool";
    if (featureNames.includes("portal") || featureNames.includes("customer") || featureNames.includes("account"))
        return "customer_portal";
    return "other";
}
// ─── Main Analysis ──────────────────────────────────────────────────
export function analyzeCodebase(rootDir) {
...
async function writeToNeo4j(analysis) {
...
RETURN c.name
    ...
    for (const model of analysis.dataModels) {
        if (!modelCategories.has(model.category))
            modelCategories.set(model.category, []);
