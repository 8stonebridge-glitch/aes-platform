 ...
 */
export async function recordBuildOutcome(neo4jRun, record) {
 ...
 */
export async function getPathSuccessScores(neo4jRun, appNames) {
    ...
    `);
        ...
        const recentSuccess = rows.slice(0, mid).filter((r) => r.succeeded).length / Math.max(1, mid);
        const olderSuccess = rows.slice(mid).filter((r) => r.succeeded).length / Math.max(1, rows.length - mid);
        let trend = "stable";
        if (recentSuccess - olderSuccess > 0.15)
            trend = "improving";
        else if (olderSuccess - recentSuccess > 0.15)
            trend = "declining";
        // Average verification score (excluding -1 sentinel)
        const vsRows = rows.filter((r) => r.vs !== null && r.vs >= 0);
        const avgVs = vsRows.length > 0
            ? vsRows.reduce((sum, r) => sum + r.vs, 0) / vsRows.length
            : 0;
        scores.set(app, {
            name: app,
            label: "LearnedApp",
 ...
 */
export async function getNodeSuccessScores(neo4jRun, nodeLabel, relType) {
    const rows = await neo4jRun(`
 ...
 */
export async function getLeaderboard(neo4jRun) {
    // Overall stats
 ...
 */
export async function getSuccessBonus(neo4jRun) {
    const bonus = new Map();
    const rows = await neo4jRun(`
  ...
  `);
    for (const r of rows) {
        ...
        if (rate >= 0.8)
            ...
            bonus.set(r.app, -2); // Penalty for unreliable sources
    }
    return bonus;
}
// ═══════════════════════════════════════════════════════════════════════
