...
async function findSeedNodes(request) {
...
function scoreEdges(edges, requestKeywords) {
    for (const edge of edges) {
        ...
        if (target.label === "LearnedDataModel")
            score += 1; // models are always valuable
        if (target.label === "LearnedPattern")
            score += 1; // patterns are reusable
        if (target.label === "LearnedUserFlow")
            score += 1; // flows guide UX
...
async function thinkOnGraph(request, maxHops = 4, beamWidth = 3) {
    ...
    const visited = new Set(); // nodes we've already explored FROM
    const seen = new Set(); // nodes we've seen at all (prevents loops)
    let currentBeam = result.seedNodes.slice(0, beamWidth);
    // Mark seeds as seen
    for (const s of currentBeam)
    ...
    for (let hop = 0; hop < maxHops; hop++) {
        ...
        if (cats.get("features").size < 3)
            hungerBonus.set("LearnedFeature", 3);
        if (cats.get("models").size < 3)
            hungerBonus.set("LearnedDataModel", 3);
        if (cats.get("integrations").size < 2)
            hungerBonus.set("LearnedIntegration", 4);
        if (cats.get("patterns").size < 3)
            hungerBonus.set("LearnedPattern", 3);
        if (cats.get("flows").size < 2)
            hungerBonus.set("LearnedUserFlow", 4);
