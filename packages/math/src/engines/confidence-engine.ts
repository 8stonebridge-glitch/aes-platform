...
export function computeConfidence(dimensions: ConfidenceDimensions): ConfidenceResult {
  ...
  for (const [key, weight] of Object.entries(CONFIDENCE_WEIGHTS)) {
    composite += dimensions[key as keyof ConfidenceDimensions] * weight;
  }
...
export function computeEvidenceCoverage(params: {
  ...
  if (params.total_claims === 0) return 0;
...
export function computeDependencyCompleteness(params: {
  ...
  if (params.total_dependencies === 0) return 1.0;
  if (params.blocked_dependencies > 0) return 0;
...
export function computeFreshness(params: {
  ...
  if (ageDays >= params.evidence_max_age_days) return 0;
...
export function computeTestCoverage(params: {
  ...
  if (params.required_tests === 0) return 1.0;
  if (params.failing_tests > 0) return Math.max(0, (params.passing_tests - params.failing_tests) / params.required_tests);
...
export function computeContradictionPenalty(params: {
  ...
  if (params.contradictions.length === 0) return 1.0;
