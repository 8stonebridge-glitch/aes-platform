...
interface BridgeRule {
  ...
  description: string;
  required: boolean;
  gate_type: "coverage" | "dependency" | "flow" | "buildability" | "contradiction" | "confidence";
}

export function validateRuleCompliance(input: ValidatorInput): ValidatorOutput {
  ...
  const rules: BridgeRule[] = bridge.applied_rules || [];
