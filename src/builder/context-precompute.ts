 ...
 */
export function precomputeContext(workspacePath: string): PrecomputedContext {
 ...
 */
export function scanRouteMap(workspacePath: string): Record<string, string> {
  const routeMap: Record<string, string> = {};
  const apiDir = join(workspacePath, "src", "app", "api");
  if (!existsSync(apiDir)) return routeMap;
 ...
 */
export function scanSchemaSummary(workspacePath: string): { tables: string[]; summary: string } {
  const tables: string[] = [];
 ...
 */
export function scanComponents(workspacePath: string): ComponentEntry[] {
  const components: ComponentEntry[] = [];
  const compDir = join(workspacePath, "src", "components");
  if (!existsSync(compDir)) return components;
...
function scanSharedUtils(workspacePath: string): string[] {
  const utils: string[] = [];
  const libDir = join(workspacePath, "src", "lib");
  if (!existsSync(libDir)) return utils;
...
function scanPages(workspacePath: string): string[] {
  const pages: string[] = [];
...
function walkDir(dir: string, callback: (filePath: string) => void, maxDepth = 6, depth = 0): void {
 ...
 */
export function updateContextAfterBuild(
