...
const CLASS_CONFIGS: Record<BuildClass, Omit<BuildClassConfig, "build_class">> = {
...
const CLASS_SIGNALS: Record<BuildClass, string[]> = {
...
export function classifyFeature(feature: {
  ...
  description?: string;
  actor_ids?: string[];
  destructive_actions?: { action_name: string }[];
  ...
  // Score each class
  const scores: Record<BuildClass, number> = {
    ui_only: 0, crud: 0, stateful: 0, auth_sensitive: 0, infra_config: 0,
  };

  for (const [cls, keywords] of Object.entries(CLASS_SIGNALS) as [BuildClass, string[]][]) {
    for (const kw of keywords) {
      if (text.includes(kw)) scores[cls] += 1;
    }
  }
  ...
  let best: BuildClass = "crud";
  let bestScore = 0;
  for (const [cls, score] of Object.entries(scores) as [BuildClass, number][]) {
    if (score > bestScore) {
      best = cls;
      bestScore = score;
    }
...
export function classifyAllFeatures(features: {
  ...
  const result = new Map<string, BuildClassConfig>();
...
export function getClassConfig(buildClass: BuildClass): BuildClassConfig {
