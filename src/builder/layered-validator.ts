...
export function validateLayer1(
  ...
  const forbiddenViolations = run.scope_violations.filter(
    v => v.includes("forbidden") || v.includes("outside")
  );
  checks.push({
  ...
  if (allowedPaths.length > 0) {
    const outOfScope = [...run.files_created, ...run.files_modified].filter(
...
export function validateLayer3(
  runs: Record<string, BuilderRunRecord>,
  bridges: Record<string, FeatureBridge>
): LayerResult {
  const start = Date.now();
  const checks: LayerCheck[] = [];

  const allCreated = new Map<string, string>(); // file -> feature_id
  const allModified = new Map<string, string[]>(); // file -> feature_ids

  // Check for file conflicts
  for (const [featureId, run] of Object.entries(runs)) {
    if (featureId === "__app__") continue;
...
export function runValidationPipeline(
  ...
  const l1Results = new Map<string, LayerResult>();
  const l2Results = new Map<string, LayerResult>();
  ...
  let l3Result: LayerResult | null = null;
  if (l1Results.size > 0) {
