...
export async function executeParallel(
  tasks: BuildTask[],
  onProgress?: (featureId: string, status: string) => void
): Promise<BuildResult[]> {
  ...
  for (const level of levels) {
    ...
    if (failedIds.length > 0) {
      ...
      for (const task of tasks) {
        if (completed.has(task.feature_id)) continue;
 ...
 */
function buildDependencyLevels(tasks: BuildTask[]): string[][] {
  const taskMap = new Map(tasks.map(t => [t.feature_id, t]));
  const levels: string[][] = [];
  ...
  while (remaining > 0 && maxIterations-- > 0) {
    ...
    for (const task of tasks) {
      if (assigned.has(task.feature_id)) continue;
