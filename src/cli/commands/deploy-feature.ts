...
export async function deployFeatureCommand(jobId: string, featureId: string) {
  ...
  if (verification.passed) {
    ...
    for (const v of verification.scope_violations) console.log(`  - [scope] ${v}`);
    for (const v of verification.constraint_violations) console.log(`  - [constraint] ${v}`);
  }
  ...
  if (!verification.passed) blockers.push("Post-build verification failed");
  if (!frameworkPassed) blockers.push("Framework validation failed");
