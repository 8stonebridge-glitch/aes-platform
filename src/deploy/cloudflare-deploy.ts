...
interface PreviewReadinessResult {
  ready: boolean;
...
async function resolveWorkersDevSubdomain(
  config: CloudflareDeployConfig,
): Promise<{ subdomain: string | null; error: string | null }> {
  const configured =
    config.workersSubdomain || process.env.AES_CF_WORKERS_SUBDOMAIN;
  if (configured) {
...
function inferAppTitle(modules: BundledModule[]): string {
  ...
  ].filter(Boolean) as BundledModule[];
...
function inferRouteSummary(content: string): string | null {
  const cleaned = stripCodeFence(decodeFileContent(content));
  const paragraph =
    cleaned.match(/<p[^>]*>(.+?)<\/p>/s)?.[1] ||
...
function collectPreviewRoutes(modules: BundledModule[]): PreviewRoute[] {
...
function previewOutputPath(route: string): string {
  return route === "/" ? "index.html" : `${route.replace(/^\//, "")}/index.html`;
}

function renderPreviewIndex(appTitle: string, routes: PreviewRoute[]): string {
...
function withPreviewAssets(modules: BundledModule[]): BundledModule[] {
  const alreadyHasStaticEntry = modules.some((module) =>
    ["index.html", "public/index.html"].includes(module.path),
  );
  ...
  const previewModules: BundledModule[] = [
    {
      path: "index.html",
  ...
  for (const route of routes.filter((candidate) => candidate.route !== "/")) {
    ...
    previewModules.push({
      ...
      content: renderPreviewRoute(
        appTitle,
        route,
        stripCodeFence(decodeFileContent(sourceModule?.content || "")),
      ),
      type: "text",
    });
  }
  ...
  try {
    ...
    if (!readiness.ready) {
      return {
        ...
        bundleSize,
        error:
          `Gateway deployed worker but preview URL is not reachable yet. ` +
...
export async function deployViaApi(
  ...
  try {
    ...
    const enableSubdomain = await fetch(
    ...
    if (!subdomain) {
      return {
        ...
        bundleSize,
        error:
          `Cloudflare worker uploaded, but no workers.dev subdomain is configured. ` +
    ...
    if (!readiness.ready) {
      return {
        ...
        bundleSize,
        error:
          `Cloudflare worker uploaded, but preview URL is not reachable yet. ` +
          `${readiness.lastError || `Last status ${readiness.lastStatus ?? "unknown"}`}`,
      };
    }
