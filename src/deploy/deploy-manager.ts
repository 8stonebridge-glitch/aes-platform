  ...
  async prepareDeploy(workspace: Workspace, config: DeployConfig): Promise<DeployResult> {
    ...
    try {
      ...
      if (config.database_url) {
        ...
        if (config.supabase_url) values["NEXT_PUBLIC_SUPABASE_URL"] = config.supabase_url;
        if (config.supabase_anon_key) values["NEXT_PUBLIC_SUPABASE_ANON_KEY"] = config.supabase_anon_key;
        if (config.service_role_key) values["SERVICE_ROLE_KEY"] = config.service_role_key;
        if (config.jwt_secret) values["JWT_SECRET"] = config.jwt_secret;
        if (config.database_url) values["DATABASE_URL"] = config.database_url;
        if (config.postgres_password) values["POSTGRES_PASSWORD"] = config.postgres_password;
        if (config.google_client_id) values["GOOGLE_CLIENT_ID"] = config.google_client_id;
        if (config.google_client_secret) values["GOOGLE_CLIENT_SECRET"] = config.google_client_secret;
        ...
        if (config.secret_key_base) values["SECRET_KEY_BASE"] = config.secret_key_base;
      ...
      writeFileSync(
        join(workspace.path, ".aes-deploy-manifest.json"),
