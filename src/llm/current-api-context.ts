...
async function fetchHermesCorrections(): Promise<string> {
  if (!HERMES_BASE_URL) return "";

  const results = await Promise.allSettled(
    HERMES_RECALL_SEEDS.map(async (seed) => {
      ...
      if (!match?.diagnosis || !match.fix_action) return null;
...
export async function getGenerationGroundTruth(): Promise<string> {
  return getGenerationGroundTruthForPacks([]);
}

export async function getGenerationGroundTruthForPacks(
