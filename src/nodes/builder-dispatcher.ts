...
import { classifyAllFeatures, type BuildClassConfig } from "../builder/feature-classifier.js";
import { validateChangePlan, buildPlanPrompt, type ChangePlan } from "../builder/two-pass-builder.js";
import { compileSlimContract, measureContractReduction } from "../builder/slim-contract.js";
import { precomputeContext, updateContextAfterBuild, type PrecomputedContext } from "../builder/context-precompute.js";
import { withTimeout } from "../builder/timeout-runner.js";
import { executeParallel, type BuildTask, type BuildResult } from "../builder/parallel-executor.js";
...
} from "../builder/worktree-isolation.js";
...
export async function builderDispatcher(
  ...
  const preflightResults = runPreflightAll(
    featureBuildOrder,
...
// ─── Helper: build a single feature with P1-P4 applied ──────────────

interface SingleFeatureBuildResult {
  featureId: string;
  featureName: string;
  run: any;
  workspace?: any;
  success: boolean;
  fixTrail: FixTrailEntry[];
  skipped?: boolean;
  skipReason?: string;
}
 ...
 */
async function buildSingleFeature(
  state: AESStateType,
  featureId: string,
  index: number,
  total: number,
  classConfig: BuildClassConfig | undefined,
  sharedContext: PrecomputedContext | null,
  existingBuildResults: Record<string, any>,
): Promise<SingleFeatureBuildResult> {
  const cb = getCallbacks();
  const store = getJobStore();
  ...
  (builderContext as any).slimContract = slimContract;

  // ── P3: Attach precomputed shared context ──
  if (sharedContext) {
    (builderContext as any).precomputed = {
  ...
  try {
    const timeoutResult = await withTimeout(
      () => codeBuilder.build(state.jobId, pkg!, undefined, builderContext),
      timeoutMs,
      `${featureName} build`,
    );
 ...
 */
async function parallelPerFeatureFallback(
  state: AESStateType,
  ...
  let sharedContext: PrecomputedContext | null = null;
  ...
  for (const fid of featureBuildOrder) {
    if (!readySet.has(fid)) {
      const feat = features.find((f: any) => f.feature_id === fid);
  ...
  let worktreePool: WorktreePool | null = null;
  try {
  ...
  const concurrencyTierMap: Record<string, "high" | "medium" | "low"> = {};
  for (const fid of readyFeatures) {
    ...
    if (cfg.max_concurrency >= 5) concurrencyTierMap[fid] = "high";
    else if (cfg.max_concurrency >= 3) concurrencyTierMap[fid] = "medium";
    else concurrencyTierMap[fid] = "low";
  }
  ...
  for (const fid of readyFeatures) {
    ...
    if (bridge?.dependencies) {
      featureDeps[fid] = bridge.dependencies
        .filter((d: any) => d.status === "required")
        .map((d: any) => d.feature_id)
        .filter((depId: string) => readySet.has(depId)); // Only deps in our build set
    } else {
      featureDeps[fid] = [];
    }
  }

  const tasks: BuildTask[] = readyFeatures.map((fid, idx) => ({
    feature_id: fid,
    feature_name: features.find((f: any) => f.feature_id === fid)?.name || fid,
    concurrency_tier: concurrencyTierMap[fid] || "medium",
    dependencies: featureDeps[fid] || [],
    execute: async (): Promise<BuildResult> => {
      const start = Date.now();
      const featureName = features.find((f: any) => f.feature_id === fid)?.name || fid;
      ...
      const result = await buildSingleFeature(
        state,
        fid,
        idx,
        readyFeatures.length,
        classConfigs.get(fid),
        sharedContext,
        buildResults,
      );
      ...
      if (result.run) {
        ...
        if (worktree) {
          (result.run as any).worktree_id = worktree.worktree_id;
          (result.run as any).worktree_branch = worktree.branch;
          (result.run as any).worktree_path = worktree.worktree_path;
        }
      }
  ...
  const parallelResults = await executeParallel(tasks, (featureId, status) => {
    const fname = features.find((f: any) => f.feature_id === featureId)?.name || featureId;
...
function generateQuickPlan(
  ...
  const planned: { path: string; action: "create"; estimated_lines: number; purpose: string }[] = [];

  // Estimate files based on capabilities
  const hasUI = pkg.included_capabilities.some(c =>
    /page|ui|view|dashboard|form|display|layout/i.test(c)
  );
  const hasData = pkg.included_capabilities.some(c =>
    /create|read|update|delete|crud|list|query|mutation/i.test(c)
  );
  const hasAuth = pkg.included_capabilities.some(c =>
    /auth|permission|role|access/i.test(c)
  );

  if (hasData) {
    planned.push(
      { path: `convex/${slug}/schema.ts`, action: "create", estimated_lines: 30, purpose: "Data schema" },
      { path: `convex/${slug}/queries.ts`, action: "create", estimated_lines: 60, purpose: "Query functions" },
      { path: `convex/${slug}/mutations.ts`, action: "create", estimated_lines: 80, purpose: "Mutation functions" },
    );
  }

  if (hasUI) {
    planned.push(
      { path: `src/app/${slug}/page.tsx`, action: "create", estimated_lines: 120, purpose: "Main page" },
      { path: `src/components/${slug}/list.tsx`, action: "create", estimated_lines: 80, purpose: "List component" },
      { path: `src/components/${slug}/form.tsx`, action: "create", estimated_lines: 100, purpose: "Form component" },
    );
  }

  if (hasAuth) {
    planned.push(
      { path: `src/lib/${slug}-permissions.ts`, action: "create", estimated_lines: 40, purpose: "Permission checks" },
    );
  }

  // Always include a test
  planned.push(
    { path: `__tests__/${slug}.test.ts`, action: "create", estimated_lines: 50, purpose: "Feature tests" },
  );
...
function makeFailedRun(jobId: string, featureId: string, featureName: string, reason: string): any {
  return {
