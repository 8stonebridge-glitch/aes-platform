  ...
  for (const featureId of Object.keys(bridges)) {
    ...
    for (const rule of graphPreventionRules) {
      ...
      const ruleTriggered = affectedPatterns.some((p: string) =>
        featureNameLower.includes(p.toLowerCase())
      );
      if (ruleTriggered) {
        vetoes.push({
    ...
    for (const fix of graphFixPatterns) {
      ...
      const bridgeRules = (bridge.applied_rules ?? []).map((r: any) =>
        (r.rule_id ?? r.name ?? "").toLowerCase()
      );
      const featureTags = [
        (bridge.feature_name ?? "").toLowerCase(),
        featureId.toLowerCase(),
        ...(bridge.dependencies ?? []).map((d: any) => (d.feature_id ?? "").toLowerCase()),
      ];

      // Check if any target failure patterns match this feature's context
      const antiPatternDetected = affectedPatterns.some((p: string) => {
        const pLower = p.toLowerCase();
        return featureTags.some((t) => t.includes(pLower)) ||
          bridgeRules.some((r: string) => r.includes(pLower));
      });
