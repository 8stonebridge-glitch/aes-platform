...
export function shouldSkipResearch(wordCount: number): boolean {
  return wordCount < routingPolicy.skipResearchBelowWords;
}

export function shouldAutoConfirm(ambiguityFlags: number, riskClass: string): boolean {
...
export function shouldParallelBuild(featureCount: number): boolean {
