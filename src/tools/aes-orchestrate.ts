...
interface ReasonerResult {
  apps: { name: string; class: string; relevance: number }[];
  features: { name: string; source: string; description: string }[];
  models: { name: string; source: string; category: string }[];
  integrations: { name: string; type: string; provider: string }[];
  patterns: { name: string; type: string; description: string }[];
...
interface FeaturePlan {
  ...
  source_evidence: string[]; // which graph apps/features informed this
}
...
async function runReasoner(query: string): Promise<ReasonerResult | null> {
  console.log("\n  \u25b8 Step 1: Running graph reasoner...");
  try {
    const output = execSync(
      `npx tsx src/tools/unified-graph-reasoner.ts "${query.replace(/"/g, '\\"')}"`,
...
function parseReasonerText(text: string): ReasonerResult {
  const apps: ReasonerResult["apps"] = [];
  const features: ReasonerResult["features"] = [];
  const models: ReasonerResult["models"] = [];
  const integrations: ReasonerResult["integrations"] = [];
  const patterns: ReasonerResult["patterns"] = [];
  const flows: ReasonerResult["flows"] = [];
  ...
  for (const m of text.matchAll(/🔧\s*UNIVERSAL:\s*(.+)/g)) {
    ...
    for (const name of patternList) {
      if (name) patterns.push({ name, type: "universal", description: "" });
    }
  }
...
function decomposeIntoFeatures(intent: string, evidence: ReasonerResult, design?: DesignEvidence): FeaturePlan[] {
  ...
  for (const domain of domains) {
    ...
    if (coreIds.has(domainId)) continue;
  ...
  if (design) {
    for (const screen of design.screens) {
      ...
      if (features.some(f => f.feature_id === screenSlug || f.name.toLowerCase().includes(screen.name.toLowerCase()))) continue;
      features.push({
...
function checkPromotionGates(feature: FeaturePlan, allFeatures: FeaturePlan[]): PromotionResult {
  const gates: PromotionResult["gate_results"] = [];
...
function computeBuildOrder(features: FeaturePlan[]): string[] {
  const order: string[] = [];
  ...
  for (const f of features) visit(f.feature_id);
  return order;
}
...
async function main() {
  ...
  const promoted = featurePlan.filter(f =>
    promotionResults.find(p => p.feature_id === f.feature_id)?.promoted
  );
  const blocked = featurePlan.filter(f =>
    !promotionResults.find(p => p.feature_id === f.feature_id)?.promoted
  );
