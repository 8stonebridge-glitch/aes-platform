...
// ═══════════════════════════════════════════════════════════════
// TYPES — Donor matching (from donor-match.ts, may not exist yet)
// ═══════════════════════════════════════════════════════════════
...
let findDonorsFn: ((query: FeatureQuery) => Promise<DonorMatch[]>) | null =
  null;

async function loadDonorMatcher(): Promise<void> {
  try {
    ...
    if (typeof mod.findDonors === "function") {
      ...
      console.warn(
        "  [donors] donor-match module found but findDonors not exported — skipping donor matching"
      );
    }
  } catch {
    console.warn(
      "  [donors] donor-match module not available — skipping donor matching"
    );
  }
}

async function findDonorsForFeature(
  feature: FeaturePlan
): Promise<DonorMatch[]> {
  if (!findDonorsFn) return [];
  try {
    const query: FeatureQuery = {
      ...
      required_integrations:
        feature.integrations.length > 0 ? feature.integrations : undefined,
      required_patterns:
        feature.patterns.length > 0 ? feature.patterns : undefined,
    };
    return await findDonorsFn(query);
  } catch (err: any) {
    console.warn(
      `    [donors] Error finding donors for ${feature.name}: ${err.message}`
    );
    return [];
  }
}
...
function extractDesignConstraintsForFeature(design: DesignEvidence, featureName: string): DesignConstraints | undefined {
  const matchingScreens = design.screens.filter(s =>
    featureName.toLowerCase().includes(s.name.toLowerCase()) ||
...
function compileLightBridge(
  ...
  const complexityLimits: Record<string, { files: number; lines: number }> = {
  ...
  const tests = feature.acceptance_criteria.map(
    (ac) => `test: ${ac.toLowerCase()}`
  );

  // Compute confidence from available evidence
  const evidenceCoverage = Math.min(
    feature.source_evidence.length / Math.max(3, feature.source_evidence.length),
    1.0
  );
...
function checkVetoes(
  feature: FeaturePlan,
  bridge: LightBridge,
  allFeatureIds: Set<string>
): { triggered: boolean; codes: string[] } {
  ...
  const hasPayment =
    nameL.includes("billing") ||
    nameL.includes("payment") ||
    descL.includes("payment");
  const hasRealtime =
    nameL.includes("real-time") ||
    nameL.includes("realtime") ||
    descL.includes("websocket");
  const hasAdmin =
    nameL.includes("admin") || descL.includes("admin");
  const hasAudit =
    nameL.includes("audit") || descL.includes("audit");
  const hasData =
    feature.data_models.length > 0;
  ...
  const input: VetoInput = {
    ...
    unresolved_dependencies: feature.dependencies.filter(
      (d) => !allFeatureIds.has(d)
    ).length,
    scope_violations: [],
...
function advanceState(
  ...
  confidence: number,
  hasVetoes: boolean
): { state: ArtifactState; transitions: string[] } {
  const stateOrder: ArtifactState[] = [
    "raw",
    "evidence_gathered",
    "derived",
    "validated",
...
const STATUS_ICONS: Record<FeatureBuildState["status"], string> = {
  pending: "  ",
  finding_donors: ">>",
  compiling_bridge: ">>",
  checking_vetoes: ">>",
...
function printStatus(state: FeatureBuildState, index: number, total: number): void {
  const icon = STATUS_ICONS[state.status];
  const vetoInfo =
    state.vetoes.length > 0 ? ` [vetoes: ${state.vetoes.length}]` : "";
  const donorInfo =
    state.donors.length > 0 ? ` [donors: ${state.donors.length}]` : "";
  const confInfo =
    state.bridge ? ` [conf: ${state.bridge.confidence.toFixed(2)}]` : "";
  const reason = state.blocking_reason ? ` -- ${state.blocking_reason}` : "";

  console.log(
    `  [${String(index + 1).padStart(2)}/${total}] [${icon}] ${state.name.padEnd(30)} ${state.status.padEnd(16)} ${state.artifact_state}${donorInfo}${confInfo}${vetoInfo}${reason}`
  );
}
...
function computeCriticalPath(
  ...
  buildStates: Map<string, FeatureBuildState>
): string[] {
  const depNodes: DependencyNode[] = features.map((f) => ({
    id: f.feature_id,
    name: f.name,
    status: (() => {
      const s = buildStates.get(f.feature_id);
      if (!s) return "pending" as const;
      if (s.status === "ready") return "completed" as const;
      if (s.status === "blocked") return "blocked" as const;
      if (s.status === "skipped") return "failed" as const;
...
function estimateComplexity(features: FeaturePlan[]): string {
  ...
  const total = features.reduce(
    (sum, f) => sum + (weights[f.priority] ?? 2),
    0
  );
...
async function recordBuildState(
  ...
  try {
    ...
    await neo4j.runCypher(
      `MERGE (b:BuildRun {intent: $intent, created_at: $created_at})
       SET b.total = $total,
           b.ready = $ready,
           b.blocked = $blocked,
           b.skipped = $skipped,
    ...
    for (const feat of manifest.features) {
      await neo4j.runCypher(
        `MERGE (f:FeatureBuild {name: $name, build_intent: $intent})
         SET f.status = $status,
...
function runOrchestrator(intent: string): OrchestrationResult | null {
  console.log("  [orchestrator] Running AES orchestrator...");
  try {
    const output = execSync(
      `npx tsx src/tools/aes-orchestrate.ts --plan-only "${intent.replace(/"/g, '\\"')}"`,
      { timeout: 180_000, maxBuffer: 10 * 1024 * 1024, cwd: process.cwd() }
    );
    ...
    console.warn(
      "  [orchestrator] Could not parse orchestrator output as JSON"
    );
    return null;
  } catch (err: any) {
    console.error(
      `  [orchestrator] Failed: ${err.message?.slice(0, 200)}`
    );
    return null;
  }
}
...
async function runAutoProgression(
  ...
  const featureMap = new Map(
    plan.feature_plan.map((f) => [f.feature_id, f])
  );
  const buildStates = new Map<string, FeatureBuildState>();
  ...
  for (let idx = 0; idx < plan.build_order.length; idx++) {
    ...
    const afterEvidence = advanceState(
      state.artifact_state as ArtifactState,
      "evidence_gathered",
      0.5,
      false
    );
    ...
    const afterBridge = advanceState(
      state.artifact_state as ArtifactState,
      "execution_ready",
      state.bridge.confidence,
      false
    );
  ...
  const features = plan.build_order
    ...
    .filter((s): s is FeatureBuildState => s !== undefined);
  ...
  const criticalPath = computeCriticalPath(
    plan.feature_plan.filter((f) => plan.build_order.includes(f.feature_id)),
    buildStates
  );
...
async function main(): Promise<void> {
  ...
  let plan: OrchestrationResult | null = null;
  ...
  if (hasPlanFlag) {
    ...
    try {
      ...
      plan = JSON.parse(raw) as OrchestrationResult;
    } catch (err: any) {
...
`);
  ...
  if (manifest.summary.ready > 0) {
    ...
    for (const f of manifest.features.filter((f) => f.status === "ready")) {
      ...
      console.log(
        `    - ${f.name} [confidence: ${conf}, donors: ${donors}, state: ${f.artifact_state}]`
      );
    }
  }
