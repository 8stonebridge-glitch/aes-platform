...
// ═══════════════════════════════════════════════════════════════════════
// TUNABLE PARAMETERS — the only thing the loop can change
// ═══════════════════════════════════════════════════════════════════════

export interface ReasonerParams {
  // Beam search
  beamWidth: number;             // how many edges to keep per hop (default: 6)
  maxHops: number;               // max graph traversal depth (default: 5)

  // Hunger thresholds — when to boost under-explored categories
  hungerFeatures: number;        // boost features when < this count (default: 8)
  hungerModels: number;          // boost models when < this count (default: 5)
  hungerIntegrations: number;    // boost integrations when < this count (default: 4)
  hungerPatterns: number;        // boost patterns when < this count (default: 4)
  hungerFlows: number;           // boost flows when < this count (default: 3)
  hungerApps: number;            // boost apps when < this count (default: 3)

  // Hunger bonus amounts
  hungerBonusFeature: number;    // bonus for feature edges (default: 3)
  hungerBonusModel: number;      // bonus for model edges (default: 3)
  hungerBonusIntegration: number; // bonus for integration edges (default: 4)
  hungerBonusPattern: number;    // bonus for pattern edges (default: 3)
  hungerBonusFlow: number;       // bonus for flow edges (default: 5)
  hungerBonusApp: number;        // bonus for app edges (default: 5)

  // Edge scoring
  keywordMatchBonus: number;     // per keyword match (default: 2)
  modelStructuralBonus: number;  // structural bonus for models (default: 1)
  patternStructuralBonus: number; // structural bonus for patterns (default: 1)
  flowStructuralBonus: number;   // structural bonus for flows (default: 1)
  complexityBonus: number;       // bonus for "complex" features (default: 1)
  sameCategoryPenalty: number;   // penalty for SAME_CATEGORY edges (default: 2)
  vectorBoostMultiplier: number; // multiply cosine sim by this for bonus (default: 4)

  // Synonym generation
  synonymCoOccurrenceMin: number; // min apps for co-occurrence (default: 2)
  synonymMinLength: number;      // min term length (default: 3)
  synonymMaxPerKeyword: number;  // max synonyms per seed keyword (default: 12)

  // RRF
  rrfK: number;                  // RRF constant (default: 60)
  dualSourceBoost: number;       // multiplier for dual-source items (default: 1.5)

  // Seed limits
  maxSeeds: number;              // max total seeds (default: 14)
  maxAppSeeds: number;           // max app seeds in initial beam (default: 8)

  // Universal pattern threshold
  universalPatternPercent: number; // min % of apps for "universal" (default: 0.4)
}

export const DEFAULT_PARAMS: ReasonerParams = {
  beamWidth: 6,
  maxHops: 5,
  hungerFeatures: 8,
  hungerModels: 5,
...
function loadParams(): ReasonerParams {
...
function saveParams(params: ReasonerParams) {
  writeFileSync(PARAMS_FILE, JSON.stringify(params, null, 2) + "\n");
}

// ═══════════════════════════════════════════════════════════════════════
// BENCHMARK SUITE — fixed test queries with expected outputs
// ═══════════════════════════════════════════════════════════════════════

interface BenchmarkCase {
  id: string;
  query: string;
  /** Apps that MUST appear in discovered knowledge */
  expectedApps: string[];
  /** Features that SHOULD appear (partial credit) */
  expectedFeatures: string[];
  /** Minimum domains that should be identified */
  minDomains: number;
  /** Minimum category coverage % */
  minCoverage: number;
  /** Weight of this case in the final score */
  weight: number;
}
...
// ═══════════════════════════════════════════════════════════════════════
// REASONER RUNNER — execute with given params
// ═══════════════════════════════════════════════════════════════════════
...
interface ReasonerOutput {
  ...
  domainCount: number;
  conceptScores: { concept: string; confidence: string }[];
 ...
 */
async function runReasoner(query: string, params: ReasonerParams): Promise<ReasonerOutput | null> {
  try {
    const result = execSync(
      `npx tsx src/tools/reasoner-bench-runner.ts ${JSON.stringify(query)}`,
      {
        env: {
          ...process.env,
...
// ═══════════════════════════════════════════════════════════════════════
// SCORING — compute a single number from reasoner output
// ═══════════════════════════════════════════════════════════════════════

interface CaseScore {
  caseId: string;
  appScore: number;       // 0-1: fraction of expected apps found
  featureScore: number;   // 0-1: fraction of expected features found (fuzzy)
  coverageScore: number;  // 0-1: category coverage vs minimum
  domainScore: number;    // 0-1: domains found vs minimum
  diversityScore: number; // 0-1: how many distinct node types discovered
  efficiencyScore: number; // 0-1: useful paths vs total hops
  composite: number;      // weighted combination
}

function scoreCase(bench: BenchmarkCase, output: ReasonerOutput): CaseScore {
  ...
  if (bench.expectedApps.length > 0) {
    for (const expected of bench.expectedApps) {
      const found = output.discoveredApps.some(a =>
...
function computeTotalScore(caseScores: CaseScore[], benchmarks: BenchmarkCase[]): number {
  let weightedSum = 0;
  let totalWeight = 0;
...
// ═══════════════════════════════════════════════════════════════════════
// MUTATION — propose random parameter changes
// ═══════════════════════════════════════════════════════════════════════

type ParamKey = keyof ReasonerParams;
...
function mutateParams(params: ReasonerParams, mutationCount: number = 2): { params: ReasonerParams; mutations: string[] } {
  const newParams = { ...params };
  const mutations: string[] = [];
  const keys = Object.keys(PARAM_RANGES) as ParamKey[];
  ...
  for (const key of toMutate) {
    ...
    let newVal = (oldVal as number) + direction * steps * range.step;
...
interface ExperimentEntry {
  ...
  accepted: boolean;
  mutations: string[];
  caseScores: { id: string; score: number }[];
  durationMs: number;
}

const LOG_FILE = "src/tools/autoresearch-log.jsonl";

function appendLog(entry: ExperimentEntry) {
...
async function runBenchmark(params: ReasonerParams): Promise<{ totalScore: number; caseScores: CaseScore[] }> {
