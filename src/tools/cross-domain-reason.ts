...
// ─── 1. MAP ALL DOMAINS IN THE GRAPH ─────────────────────────────────
...
interface AppDomain {
  ...
  flowCount: number;
  totalNodes: number;
  strengths: string[];  // what this app is best at
}

async function mapAllDomains(): Promise<AppDomain[]> {
  const apps = await q(`
    MATCH (a:LearnedApp)
...
// ─── 2. IDENTIFY WHICH DOMAINS A REQUEST TOUCHES ────────────────────
...
function identifyDomains(request: string): DomainMatch[] {
  ...
  const domainRules: { domain: string; triggers: string[]; desc: string; keywords: string[] }[] = [
...
// ─── 3. FIND BEST SOURCE APP FOR EACH DOMAIN ────────────────────────
...
// ─── 4. FIND UNIVERSAL PATTERNS (appear in 60%+ of apps) ────────────
...
async function findUniversalPatterns(): Promise<UniversalPattern[]> {
...
async function findCrossDomainModels(): Promise<{ name: string; categories: string[]; appClasses: string[] }[]> {
...
// ─── 6. GENERATE CROSS-DOMAIN BLUEPRINT ─────────────────────────────
...
async function main() {
  ...
  const usedApps = new Map<string, string[]>();
  for (const source of sources) {
    if (source.score > 0) {
      if (!usedApps.has(source.bestApp)) usedApps.set(source.bestApp, []);
