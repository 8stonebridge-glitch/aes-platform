...
async function linkFeatures(dryRun: boolean): Promise<number> {
  ...
  const features: FeatureNode[] = rows.map((r: any) => ({
  ...
  const bySource = new Map<string, FeatureNode[]>();
  ...
  const pairs: Array<{ a: FeatureNode; b: FeatureNode; score: number }> = [];
  ...
  if (pairs.length > 0 && !dryRun) {
    ...
    if (pairs.length > 5) console.log("    ... and %d more", pairs.length - 5);
  }
...
async function linkModels(dryRun: boolean): Promise<number> {
  const neo4j = getNeo4jService();
  ...
  const models: ModelNode[] = rows.map((r: any) => ({
    name: r.name,
    source: r.source,
  ...
  for (const m of models) {
    if (!byName.has(m.normalized)) byName.set(m.normalized, []);
    byName.get(m.normalized)!.push(m);
  }

  // Find names that appear in multiple different sources
  const pairs: Array<{ a: ModelNode; b: ModelNode }> = [];
  for (const [, group] of byName) {
    // Deduplicate by source within the group
    const uniqueSources = new Map<string, ModelNode>();
    for (const m of group) {
      if (!uniqueSources.has(m.source)) uniqueSources.set(m.source, m);
    }
...
async function linkIntegrations(dryRun: boolean): Promise<number> {
  const neo4j = getNeo4jService();
  ...
  const integrations: IntegrationNode[] = rows.map((r: any) => ({
    name: r.name,
    source: r.source,
  ...
  console.log(
    "  Loaded %d integrations from %d apps",
    integrations.length,
    new Set(integrations.map((i) => i.source)).size
  );

  // Group by normalized provider
  const byProvider = new Map<string, IntegrationNode[]>();
  ...
  const pairs: Array<{ a: IntegrationNode; b: IntegrationNode }> = [];
  for (const [, group] of byProvider) {
    const uniqueSources = new Map<string, IntegrationNode>();
    for (const i of group) {
      if (!uniqueSources.has(i.source)) uniqueSources.set(i.source, i);
    }
    const unique = [...uniqueSources.values()];
    if (unique.length < 2) continue;
...
async function linkByEmbeddings(dryRun: boolean): Promise<number> {
  const neo4j = getNeo4jService();
  ...
  if (dryRun) {
    ...
    if (preview.length > 5) console.log("    ... and %d more", preview.length - 5);
...
async function main() {
  ...
  if (useEmbeddings) console.log("  Embedding similarity: ENABLED");
  ...
  try {
    ...
    // ── Summary ──
    const total =
      stats.featureLinks + stats.modelLinks + stats.integrationLinks + stats.embeddingLinks;
