...
async function main(): Promise<void> {
  ...
  const evidenceList: DesignEvidence[] = [];
  ...
  if (jsonFile) {
    ...
    const isNormalized =
      parsed.screens?.every((s: any) => typeof s.is_overlay === "boolean") &&
      parsed.components?.every((c: any) => typeof c.purpose === "string");

    if (isNormalized) {
      // Already canonical — re-persist as-is
      evidenceList.push(parsed as DesignEvidence);
    } else {
      // Raw — normalize first
      evidenceList.push(normalizeDesignEvidence(parsed as RawDesignEvidence));
    }
  } else {
    ...
    if (files.length === 0) {
      ...
      console.log(
        "[backfill] Use --json <file> to specify a file, or run from the directory containing evidence JSON."
      );
      process.exit(0);
    }
    ...
    for (const file of files) {
      ...
      const normalized = normalizeDesignEvidence(parsed as RawDesignEvidence);
  ...
  for (const evidence of evidenceList) {
    ...
    try {
      ...
      console.error(
        `  Failed to persist: ${(err as Error).message}`
      );
    }
  }
