...
async function q(cypher: string): Promise<any[]> {
  ...
  catch (e: any) { console.error(`[neo4j] ${e.message.slice(0, 120)}`); return []; }
}
...
// ─── What Perplexity says a barber booking app needs ─────────────────
// (same ground truth, but now we search the graph smartly)

interface Requirement {
  name: string;
  keywords: string[];  // multiple search terms to try
  type: "feature" | "model" | "integration" | "pattern" | "flow" | "ui";
}
...
async function searchGraph(req: Requirement): Promise<{ found: boolean; evidence: string }> {
...
async function main() {
  ...
  for (const req of REQUIREMENTS) {
    if (!groups.has(req.type)) groups.set(req.type, []);
  ...
  let totalFound = 0;
  let totalRequired = 0;
  const sectionResults: { label: string; found: number; total: number; hits: string[]; misses: string[] }[] = [];
