...
import {
  embedBatch, isEmbeddingAvailable, getEmbeddingDimensions,
  featureText, modelText, integrationText, patternText, flowText,
...
async function main() {
  ...
  await embedNodeType(neo4j, {
    ...
    textFn: (r: any) => featureText({ name: r.name, description: r.description, complexity: r.complexity, has_api: r.has_api, has_tests: r.has_tests, directory: r.directory, app_name: r.app_name, app_class: r.app_class }),
    setQuery: (id: string) => `MATCH (f:LearnedFeature {feature_id: $id}) SET f.embedding = $embedding`,
    stats,
  });
  ...
  await embedNodeType(neo4j, {
    ...
    textFn: (r: any) => modelText({ name: r.name, category: r.category, fields_csv: r.fields_csv, app_name: r.app_name, app_class: r.app_class, relation_count: typeof r.relation_count === "number" ? r.relation_count : undefined }),
    setQuery: (id: string) => `MATCH (m:LearnedDataModel {name: $id}) SET m.embedding = $embedding`,
    stats,
  });
  ...
  await embedNodeType(neo4j, {
    ...
    textFn: (r: any) => integrationText({ name: r.name, type: r.type, provider: r.provider, app_name: r.app_name, auth_method: r.auth_method }),
    setQuery: (id: string) => `MATCH (i:LearnedIntegration {name: $id}) SET i.embedding = $embedding`,
    stats,
  });
  ...
  await embedNodeType(neo4j, {
    ...
    textFn: (r: any) => patternText({ name: r.name, type: r.type, description: r.description, app_name: r.app_name, applicable_to: r.applicable_to }),
    setQuery: (id: string) => `MATCH (p:LearnedPattern {name: $id}) SET p.embedding = $embedding`,
    stats,
  });
  ...
  await embedNodeType(neo4j, {
    ...
    textFn: (r: any) => flowText({ name: r.name, steps_description: r.steps_description, app_name: r.app_name, section: r.section, step_count: typeof r.step_count === "number" ? r.step_count : undefined }),
    setQuery: (id: string) => `MATCH (f:LearnedUserFlow {name: $id}) SET f.embedding = $embedding`,
    stats,
  });
...
async function embedNodeType(
  ...
  opts: {
    ...
    setQuery: (id: string) => string;
    stats: { total: number; embedded: number; skipped: number; failed: number };
  },
) {
