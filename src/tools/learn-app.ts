...
function findFiles(dir: string, re: RegExp, maxDepth = 6, d = 0): string[] {
...
function findDirs(dir: string, re: RegExp, maxDepth = 5, d = 0): string[] {
  if (d >= maxDepth) return [];
  const out: string[] = [];
  try {
    for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
...
function scanFeatures(root: string): LearnedFeature[] {
  ...
  for (const pat of featureDirPatterns) {
    ...
    for (const d of found) {
      ...
      if (!fixedDirs.includes(rel)) fixedDirs.push(rel);
    }
  }
...
function scanDataModels(root: string): LearnedDataModel[] {
  ...
  for (const df of drizzleFiles) {
    ...
    if (rel.includes("node_modules") || rel.includes("zod")) continue;
...
function classifyIntType(name: string): IntegrationType {
  const l = name.toLowerCase();
  if (/video|zoom|meet|teams|jitsi|daily/.test(l)) return "video_conferencing";
  if (/calendar|caldav|ical/.test(l)) return "calendar";
  if (/payment|stripe|paypal|btcpay|billing/.test(l)) return "payment";
  if (/crm|hubspot|salesforce|pipedrive/.test(l)) return "crm";
  if (/analytics|ga4|posthog|plausible|mixpanel|amplitude/.test(l)) return "analytics";
  if (/zapier|make|n8n|automation/.test(l)) return "automation";
  if (/slack|discord|telegram|whatsapp/.test(l)) return "messaging";
  if (/email|sendgrid|resend|mailgun|postmark|ses/.test(l)) return "email";
  if (/sms|twilio/.test(l)) return "sms";
  if (/s3|storage|upload|minio|r2/.test(l)) return "storage";
  if (/sentry|datadog|grafana|prometheus/.test(l)) return "monitoring";
  if (/auth0|clerk|next-auth|lucia|passport|keycloak/.test(l)) return "auth";
  if (/aws|gcp|azure|cloud|vercel|fly/.test(l)) return "cloud";
  return "other";
}

function inferAuthMethod(name: string, content: string): "oauth" | "api_key" | "webhook" | "unknown" {
  if (/OAuth|oauth|getToken|refreshToken|authorization_code/.test(content)) return "oauth";
  if (/apiKey|API_KEY|api_key|secret_key|bearer/.test(content)) return "api_key";
  if (/webhook|Webhook/.test(content)) return "webhook";
  return "unknown";
}
...
function scanIntegrations(root: string, deps: string[]): LearnedIntegration[] {
  ...
  for (const fixed of ["packages/app-store", "src/integrations", "packages/integrations"]) {
    ...
    if (exists(fp) && !intDirs.includes(fp)) intDirs.push(fp);
  }
...
function scanApi(root: string, deps: string[]): LearnedApiSurface {
  ...
  if (has(deps, "graphql", "@apollo", "type-graphql", "@nestjs/graphql")) style = style === "rest" ? "graphql" : "mixed";
    ...
    const scanR = (dir: string, prefix: string) => {
      try {
        for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
          ...
          if (/\bGET\b|export.*GET/.test(c)) methods.push("GET");
  ...
  for (const full of trpcDirs) {
    const rel = path.relative(root, full);
    if (rel.includes("node_modules") || !(/trpc|server/.test(rel))) continue;
    const scanT = (dir: string, prefix: string) => {
      try {
        for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
  ...
  for (const cf of controllerFiles) {
    ...
    const methods: string[] = [];
    if (/@Get\b/.test(content)) methods.push("GET");
    if (/@Post\b/.test(content)) methods.push("POST");
    if (/@Put\b|@Patch\b/.test(content)) methods.push("PUT");
    if (/@Delete\b/.test(content)) methods.push("DELETE");
    if (methods.length === 0) methods.push("GET");
...
function catComponent(fp: string, name: string): string {
  ...
  if (/chart|graph|stats|metric/.test(l)) return "data_viz";
  return "general";
}
...
function scanUI(root: string, deps: string[]): LearnedUI {
  ...
  for (const fixed of ["packages/ui", "packages/coss-ui", "src/components", "apps/web/components"]) {
    const fp = path.join(root, fixed);
    if (exists(fp) && !compDirs.includes(fp)) compDirs.push(fp);
  }
  ...
  for (const full of compDirs) {
    ...
    for (const fp of findFiles(full, /\.(tsx|vue|svelte)$/, 4)) {
      ...
      if (name.startsWith("_") || name === "index" || /\.test|\.stories|\.spec/.test(name)) continue;
  ...
  for (const full of pageDirs) {
    const rel = path.relative(root, full);
    if (rel.includes("node_modules") || rel.includes("api/")) continue;

    const scanP = (dir: string, route: string, section: string) => {
      try {
        for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
          if (e.name.startsWith(".") || SKIP_DIRS.has(e.name) || e.name === "_components" || e.name === "api") continue;
          const fp = path.join(dir, e.name);
          if (e.isDirectory()) {
            ...
            if (e.name.startsWith("layout")) continue; // layouts aren't pages
            const c = readFile(fp, 30);
            ...
            if (!sectionMap.has(sec)) sectionMap.set(sec, { routes: [], isPublic: false, hasAuth: false });
  ...
  for (const f of allNavFiles) {
    ...
    if (/KBar|cmdk|CommandPalette|command-palette/.test(c)) hasCmd = true;
    if (/[Mm]obile[Nn]av|MobileNavigation|mobile-nav/.test(c)) hasMobile = true;
  ...
  const navStyle = allNavFiles.some(f => /sidebar|side-bar/i.test(f) || /SideBar|sidebar/i.test(readFile(f, 50))) ? "sidebar" as const : "topnav" as const;
  ...
  if (twConfigs.length > 0) cssFramework = "Tailwind CSS";
  else if (has(deps, "@vanilla-extract")) cssFramework = "Vanilla Extract";
  else if (has(deps, "styled-components")) cssFramework = "Styled Components";
  ...
  if (has(deps, "@headlessui")) compLib = "Headless UI";
  if (has(deps, "antd", "@ant-design")) compLib = "Ant Design";
  if (has(deps, "@shadcn")) compLib = "shadcn/ui + Radix";
  ...
  else if (has(deps, "@tabler/icons")) iconLib = "Tabler Icons";
  else if (has(deps, "react-icons")) iconLib = "React Icons";
  ...
  for (const cssFile of cssFiles.slice(0, 50)) { // limit to avoid scanning too many
    ...
    for (const v of vars) {
      ...
      if (/color|bg|brand|border|text|primary|secondary|accent|muted|foreground|background/.test(n)) {
        colorCount++; colorCats.add(n.split("-")[0]);
      }
    }
    if (/\.dark|dark-mode|data-theme|prefers-color-scheme:\s*dark/.test(c)) hasDark = true;
    if (/--brand|customBrand|brandColor|--primary/.test(c)) hasTheming = true;
  }
  ...
  const flowPatterns: [RegExp, string, string, string[]][] = [
  ...
  for (const fd of allFeatureDirs) {
    try {
      for (const e of fs.readdirSync(fd, { withFileTypes: true })) {
        if (e.isDirectory()) allDirNames.add(e.name.toLowerCase());
      }
    } catch {}
  }
...
export function analyzeApp(rootDir: string, sourceUrl = ""): LearnedApp {
  ...
  if (has(deps, "i18next", "react-intl", "next-intl", "@lingui")) archPatterns.push({ name: "Internationalization", type: "localization", description: "Multi-language support with translation system", evidence: "i18n library in dependencies", applicable_to: ["all"], key_files: [] });
...
async function main() {
  ...
  if (!exists(rootDir)) { console.error(`Path not found: ${rootDir}`); process.exit(1); }
