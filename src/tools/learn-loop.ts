...
interface TestScenario {
  description: string;
  expected: {
    stack?: string[];        // expected tech stack items
    features?: string[];     // expected feature keywords
    models?: string[];       // expected model names
    patterns?: string[];     // expected pattern names
    integrations?: string[]; // expected integration providers
    auth?: string[];         // expected auth patterns
    flows?: string[];        // expected flow keywords
    ui?: string[];           // expected UI items
  };
  corrections: {
    section: string;
    correction: string;
...
async function q(cypher: string): Promise<any[]> {
  try { return await neo4j.runCypher(cypher); } catch { return []; }
}
...
async function runScenario(scenario: TestScenario): Promise<{ pass: number; fail: number; score: number }> {
  ...
  totalScore += stackScore; sectionCount++;
  ...
  if (featRows.length === 0) featRows = await q(`MATCH (f:LearnedFeature) WHERE f.complexity = 'complex' RETURN f.name AS feature, f.complexity AS complexity ORDER BY f.file_count DESC LIMIT 20`);
  ...
  totalScore += featScore; sectionCount++;
  ...
  if (modelRows.length === 0) modelRows = await q(`MATCH (m:LearnedDataModel) WHERE m.category IN ['auth_identity','organization','payments','scheduling'] RETURN DISTINCT m.name AS model, m.category AS category, m.field_count AS fields ORDER BY m.field_count DESC LIMIT 15`);
  ...
  totalScore += modelScore; sectionCount++;
  ...
  totalScore += patScore; sectionCount++;
  ...
  totalScore += intScore; sectionCount++;
  ...
  totalScore += authScore; sectionCount++;
  ...
  totalScore += flowScore; sectionCount++;
  ...
  for (const r of results) {
    ...
    if (r.matched.length > 0) console.log(`     ✓ Found: ${r.matched.join(", ")}`);
    if (r.missed.length > 0) console.log(`     ✗ Missing: ${r.missed.join(", ")}`);
    if (r.score >= 3) pass++; else fail++;
  }
