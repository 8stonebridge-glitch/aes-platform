...
async function migrate() {
  ...
  try {
    ...
    const nodesResult = await readSession.run(
    ...
    const labelGroups = new Map<string, typeof nodes>();
      ...
      for (let i = 0; i < group.length; i += BATCH_SIZE) {
        ...
        await writeSession.run(
          ...
          for (const b of batch) {
            await writeSession.run(
              `CREATE (n:${labels}) SET n = $props, n._migrationId = $id`,
              { props: b.props, id: neo4j.int(b.id) }
            );
          }
        });
    ...
    const relsResult = await relSession.run(
      `MATCH (a)-[r]->(b)
    ...
    const relGroups = new Map<string, typeof rels>();
    for (const rel of rels) {
      if (!relGroups.has(rel.relType)) relGroups.set(rel.relType, []);
    ...
    for (const [relType, group] of relGroups) {
      ...
      for (let i = 0; i < group.length; i += BATCH_SIZE) {
        ...
        for (const rel of batch) {
          await writeSession.run(
            `MATCH (a {_migrationId: $fromId}), (b {_migrationId: $toId})
    ...
    for (const idx of indexes) {
      if (idx.type === "LOOKUP") continue; // built-in
