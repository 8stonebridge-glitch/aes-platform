...
const SOURCE_TO_APP: Record<string, string> = {
...
const LABEL_TO_REL: Record<string, string> = {
...
async function main() {
  ...
  const existingApps = await neo4j.runCypher(
    "MATCH (a:LearnedApp) RETURN a.name AS name"
  );
  ...
  for (const appName of neededApps) {
    if (!existingNames.has(appName)) {
      ...
      await neo4j.runCypher(
        "CREATE (a:LearnedApp {name: $name, source: 'reconnect-orphans', created_at: $now})",
        { name: appName, now: new Date().toISOString() }
      );
      totalCreatedApps++;
    }
  }
  ...
  for (const [source, appName] of Object.entries(SOURCE_TO_APP)) {
    ...
    const orphans = await neo4j.runCypher(
      `MATCH (n) WHERE NOT (n)--() AND n.source = $source
  ...
  const perplexityOrphans = await neo4j.runCypher(
    `MATCH (n) WHERE NOT (n)--() AND n.source = 'perplexity-research'
     RETURN elementId(n) AS id, labels(n)[0] AS label, count(*) AS cnt`,
  );
  // Group perplexity orphans by their app_description or domain
  const perplexityNodes = await neo4j.runCypher(
    `MATCH (n) WHERE NOT (n)--() AND n.source = 'perplexity-research'
     RETURN elementId(n) AS id, labels(n)[0] AS label,
  ...
  if ((perplexityNodes as any[]).length > 0) {
    console.log(`\nPerplexity research orphans: ${(perplexityNodes as any[]).length}`);

    // Create a research hub node
    await neo4j.runCypher(
      `MERGE (h:ResearchHub {name: 'perplexity-research'})
       ON CREATE SET h.created_at = $now, h.source = 'reconnect-orphans'`,
    ...
    for (const label of ['LearnedCorrection', 'LearnedResearch', 'LearnedFeedback']) {
      ...
      const result = await neo4j.runCypher(
        `MATCH (h:ResearchHub {name: 'perplexity-research'})
  ...
  const buildOrphans = await neo4j.runCypher(
    `MATCH (n:BuildRecord) WHERE NOT (n)--()
     RETURN elementId(n) AS id, n.source AS source`
  );
  if ((buildOrphans as any[]).length > 0) {
    console.log(`\nBuildRecord orphans: ${(buildOrphans as any[]).length}`);
    await neo4j.runCypher(
      `MERGE (h:BuildHistory {name: 'build-records'})
    ...
    totalCreatedRels += (buildOrphans as any[]).length;
    console.log(`  Connected ${(buildOrphans as any[]).length} BuildRecords to BuildHistory hub`);
  }
  ...
  const remaining = await neo4j.runCypher(
    `MATCH (n) WHERE NOT (n)--()
