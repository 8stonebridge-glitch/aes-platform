...
async function q(cypher: string): Promise<any[]> {
  ...
  catch (e: any) { console.error(`[neo4j] Query failed: ${e.message}`); return []; }
}
...
interface ResearchResult {
  ...
  features: { name: string; description: string; complexity: "simple" | "moderate" | "complex" }[];
  data_models: { name: string; category: string; fields: string }[];
  integrations: { name: string; type: string; provider: string; auth_method: string }[];
  auth_patterns: { name: string; description: string }[];
  tech_stack: { name: string; role: string }[];
  user_flows: { name: string; steps: string }[];
  ui_patterns: { name: string; description: string }[];
}
 ...
 */
function buildResearchPrompt(appDescription: string): string {
 ...
 */
function parseResearchResponse(raw: string): ResearchResult | null {
 ...
 */
async function writeToGraph(research: ResearchResult): Promise<{
  features: number;
  models: number;
  integrations: number;
  patterns: number;
...
async function researchViaAPI(appDescription: string): Promise<string> {
  const apiKey = process.env.PERPLEXITY_API_KEY;
  if (!apiKey) {
    throw new Error(
      "PERPLEXITY_API_KEY not set. Either:\n" +
...
async function verifyBackfill(appId: string): Promise<void> {
