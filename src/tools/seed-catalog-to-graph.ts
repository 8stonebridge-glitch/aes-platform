...
interface CatalogYaml {
  ...
  donor_lineage: string[];
  last_validation_date: string;
  tests?: { suite: string; last_run: string; status: string };
}

function loadAllYamls(): { entry: CatalogYaml; category: string; file: string }[] {
  const results: { entry: CatalogYaml; category: string; file: string }[] = [];
  ...
  for (const { dir, category } of CATALOG_DIRS) {
    ...
    for (const f of files) {
      try {
        ...
        const parsed = parseYaml(content) as CatalogYaml;
...
async function main() {
  ...
  try {
    await neo4j.runCypher(
      `CREATE CONSTRAINT catalog_entry_id IF NOT EXISTS FOR (c:CatalogEntry) REQUIRE c.id IS UNIQUE`
    );
...
RETURN count(*) as cnt
`);
        if (result.length > 0) domainLinks++;
      } catch {
  ...
  const uiPatternMappings: Record<string, string[]> = {
