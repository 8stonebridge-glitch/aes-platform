...
export interface UnifiedResult {
  ...
  // From cross-domain decomposition
  domains: DomainMatch[];
  domainSources: DomainSource[];

  // From auto-synonym generation
  synonymClusters: Map<string, string[]>;
  expandedKeywords: string[];

  // From Think-on-Graph beam search
  seedNodes: GraphNode[];
  hops: HopResult[];
  ...
  universalPatterns: { name: string; type: string; percentage: number }[];

  // From hybrid RRF search
...
async function loadReasoningRules(): Promise<string[]> {
  const rules = await q(`
    MATCH (r:AESReasoningRule)
    RETURN r.title AS title, r.summary AS summary
    ORDER BY r.priority
  `);
  return rules.map((r: any) => r.title || r.summary);
}
...
const DOMAIN_RULES: { domain: string; triggers: string[]; desc: string; keywords: string[] }[] = [
...
async function findBestSourceForDomain(domain: DomainMatch): Promise<DomainSource> {
  ...
  return {
    ...
    alternateApps: appScores.slice(1).map((r: any) => r.app),
  };
}
...
async function hybridSearch(
  ...
  const labelToIdField: Record<string, string> = {
  ...
  const keywordRanked: RankedItem[] = keywordRows.map((r: any) => ({
    id: r.id || r.name,
    name: r.name,
    label: nodeType,
    properties: r.props || {},
 ...
 */
async function hybridSearchAll(
  queryText: string,
  keywords: string[],
  topKPerType: number = 8,
): Promise<Map<string, FusedResult[]>> {
  const types = [
    "LearnedFeature", "LearnedDataModel", "LearnedIntegration",
    "LearnedPattern", "LearnedUserFlow",
  ];
  const resultMap = new Map<string, FusedResult[]>();
  const searchPromises: Promise<void>[] = [];
  for (const t of types) {
    searchPromises.push(
      hybridSearch(queryText, keywords, t, topKPerType).then(results => {
...
async function findSeedNodes(
  ...
  for (const domain of domains) {
    ...
    if (!found) {
      const kwFilter = domain.keywords.slice(0, 5).map(kw =>
        `toLower(f.name) CONTAINS '${esc(kw)}'`).join(" OR ");
      const apps = await q(`
  ...
  for (const kw of rawWords) {
    ...
    for (const r of features) {
      ...
      if (GENERIC_FEATURES.has(r.name.toLowerCase())) continue;
...
async function discoverEdges(node: GraphNode): Promise<GraphEdge[]> {
  ...
  if (node.label === "LearnedApp") {
    ...
    for (const r of features) edges.push({ type: "HAS_FEATURE", targetNode: { id: r.id, label: "LearnedFeature", name: r.name, properties: { complexity: r.complexity, description: r.desc } }, score: 0, reason: "" });
    for (const r of models) edges.push({ type: "HAS_DATA_MODEL", targetNode: { id: r.name, label: "LearnedDataModel", name: r.name, properties: { category: r.category, fields: r.fields } }, score: 0, reason: "" });
    for (const r of integrations) edges.push({ type: "HAS_INTEGRATION", targetNode: { id: r.name, label: "LearnedIntegration", name: r.name, properties: { type: r.type, provider: r.provider, auth_method: r.auth } }, score: 0, reason: "" });
    for (const r of patterns) edges.push({ type: "USES_PATTERN", targetNode: { id: r.name, label: "LearnedPattern", name: r.name, properties: { type: r.type, description: r.desc } }, score: 0, reason: "" });
    for (const r of flows) edges.push({ type: "HAS_USER_FLOW", targetNode: { id: r.name, label: "LearnedUserFlow", name: r.name, properties: { steps: r.steps, step_count: r.stepCount } }, score: 0, reason: "" });
  }
  ...
  if (node.label === "LearnedPattern") {
    ...
    `);
    for (const r of apps) edges.push({ type: "USED_BY_APP", targetNode: { id: r.id, label: "LearnedApp", name: r.name, properties: { app_class: r.appClass } }, score: 0, reason: "" });
  }
...
let llmScorer: LLMEdgeScorer | null = null;

/** Call this to enable LLM-based edge re-ranking in the beam search */
export function setLLMScorer(scorer: LLMEdgeScorer) {
  llmScorer = scorer;
}
...
let vectorSimCache = new Map<string, number>();

async function warmVectorCache(request: string) {
...
function scoreEdges(edges: GraphEdge[], allKeywords: string[], hungerBonus: Map<string, number>): GraphEdge[] {
  for (const edge of edges) {
    ...
    // Structural bonuses
    if (target.label === "LearnedDataModel") score += 1;
    if (target.label === "LearnedPattern") score += 1;
    if (target.label === "LearnedUserFlow") score += 1;
    ...
    if (edge.type === "SAME_CATEGORY") score -= 2;
    ...
    const parts: string[] = [];
    if (matchedKws.length > 0) parts.push(`kw: ${matchedKws.slice(0, 3).join(", ")}`);
    if (vecBonus > 0) parts.push(`vec: +${vecBonus}`);
    if (bonus > 0) parts.push(`hunger: +${bonus}`);
 ...
 */
async function maybeReRankWithLLM(request: string, edges: GraphEdge[]): Promise<GraphEdge[]> {
  if (!llmScorer || edges.length === 0) return edges;
...
function recordDiscovery(knowledge: Map<string, Set<string>>, node: GraphNode) {
  switch (node.label) {
    case "LearnedApp": knowledge.get("apps")!.add(node.name); break;
    case "LearnedFeature": knowledge.get("features")!.add(node.name); break;
    case "LearnedDataModel": knowledge.get("models")!.add(`${node.name} (${node.properties.category || "general"})`); break;
    case "LearnedIntegration": knowledge.get("integrations")!.add(`${node.name} [${node.properties.type || "other"}]`); break;
    case "LearnedPattern": knowledge.get("patterns")!.add(`${node.name} [${node.properties.type || "unknown"}]`); break;
    case "LearnedUserFlow": knowledge.get("flows")!.add(node.name); break;
  }
}
...
async function beamSearch(
  ...
  maxHops: number,
  beamWidth: number,
): Promise<{ hops: HopResult[]; discoveredKnowledge: Map<string, Set<string>>; tracedPaths: string[] }> {
  ...
  for (let hop = 0; hop < maxHops; hop++) {
    ...
    if (cats.get("features")!.size < 8)       hungerBonus.set("LearnedFeature", 3);
    if (cats.get("models")!.size < 5)          hungerBonus.set("LearnedDataModel", 3);
    if (cats.get("integrations")!.size < 4)    hungerBonus.set("LearnedIntegration", 4);
    if (cats.get("patterns")!.size < 4)        hungerBonus.set("LearnedPattern", 3);
    if (cats.get("flows")!.size < 3)           hungerBonus.set("LearnedUserFlow", 5);
    // Apps are the most valuable — each one opens 50+ edges.
    // Keep hungry for apps until we've explored enough.
    if (cats.get("apps")!.size < 3)            hungerBonus.set("LearnedApp", 5);
...
async function findUniversalPatterns(): Promise<{ name: string; type: string; percentage: number }[]> {
...
export async function unifiedReason(request: string): Promise<UnifiedResult> {
  ...
  result.hybridSeedCount = result.seedNodes.filter(s =>
    s.properties.source?.startsWith("hybrid-rrf")).length;
  ...
  const usedApps = new Map<string, { domains: string[]; features: string[]; models: string[]; integrations: string[] }>();
  for (const source of result.domainSources) {
    if (source.score > 0) {
      if (!usedApps.has(source.bestApp)) usedApps.set(source.bestApp, { domains: [], features: [], models: [], integrations: [] });
  ...
  for (const [app, data] of usedApps) {
    ...
    if (data.features.length > 0) lines.push(`  features: ${[...new Set(data.features)].slice(0, 6).join(", ")}`);
    if (data.models.length > 0) lines.push(`  models: ${[...new Set(data.models)].slice(0, 6).join(", ")}`);
    if (data.integrations.length > 0) lines.push(`  integrations: ${[...new Set(data.integrations)].slice(0, 4).join(", ")}`);
...
async function main() {
  ...
  for (const [category, items] of result.discoveredKnowledge) {
    if (items.size > 0) {
      ...
      for (const item of arr.slice(0, 8)) console.log(`      • ${item}`);
  ...
  for (const source of result.domainSources) {
    ...
    if (source.matchedFeatures.length > 0) console.log(`       Features: ${source.matchedFeatures.slice(0, 5).join(", ")}`);
    if (source.matchedModels.length > 0) console.log(`       Models:   ${source.matchedModels.slice(0, 5).join(", ")}`);
    if (source.alternateApps.length > 0) console.log(`       Also in:  ${source.alternateApps.slice(0, 3).join(", ")}`);
  }
  ...
  for (const score of result.conceptScores) {
    ...
    if (score.evidence.length > 0) console.log(`             ${score.evidence.slice(0, 5).join("; ")}`);
  }
