...
} from "../types/pattern-requirements.js";
...
function checkMarkers(content: string, markers: string[]): boolean {
 ...
 */
function inferPatternFromPath(filePath: string): string | null {
  const p = filePath.toLowerCase();
  if (p.includes("review-queue")) return "data-table-page";
  if (p.includes("audit")) return "audit-log-page";
  if (p.includes("select-role")) return "role-selection-page";
  if (p.includes("requests/page") || p.includes("submit")) return "form-page";
  if (p.includes("[id]")) return "detail-page";
...
export function validateComposition(
  ...
  const patternsToCheck = new Set<string>();
  ...
  for (const file of files) {
    ...
    if (!file.path.includes("app/")) continue;
    ...
    if (inferred) patternsToCheck.add(inferred);
  }
  ...
  // Calculate score
  const totalChecks =
    sectionsRequired + statesRequired + interactionsRequired + richnessTotal;
  const totalPassed =
    sectionsFound + statesFound + interactionsFound + richnessPass;
