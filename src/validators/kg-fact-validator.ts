...
export interface FactVerdict {
  ...
  evidence: string[];  // graph paths that support or contradict
  graphHits: number;   // how many graph nodes matched
}

export interface KGFactValidatorResult {
  verdict: "PASS" | "PASS_WITH_CONCERNS" | "FAIL";
  score: number;  // 0-1, percentage of verified claims
  total_claims: number;
  verified: number;
  unverified: number;
  contradicted: number;
...
export function extractClaims(builderOutput: {
  features?: { name: string; description?: string; data_models?: string[]; integrations?: string[]; patterns?: string[] }[];
  models?: { name: string; fields?: string[]; category?: string; relationships?: string[] }[];
  integrations?: { name: string; type?: string; provider?: string; auth_method?: string }[];
  patterns?: { name: string; type?: string }[];
  flows?: { name: string; steps?: string[] }[];
  auth?: { method?: string; provider?: string; roles?: string[] };
}): FactClaim[] {
 ...
 */
export function extractClaimsFromText(text: string, source: string): FactClaim[] {
  const claims: FactClaim[] = [];
...
async function q(neo4j: Neo4jQueryable, cypher: string): Promise<any[]> {
 ...
 */
async function verifyClaim(neo4j: Neo4jQueryable, claim: FactClaim): Promise<FactVerdict> {
  ...
  switch (claim.category) {
    case "feature": {
      ...
      hits.forEach((r: any) => evidence.push(`Feature:${r.name} [${r.complexity || "unknown"}]`));
      ...
      if (graphHits === 0) {
        ...
        synonymHits.forEach((r: any) => evidence.push(`Feature:${r.name} (via description)`));
      }
      break;
    }
    ...
    case "model": {
      if (claim.predicate === "IS_MODEL") {
        ...
        `);
        graphHits = hits.length;
        hits.forEach((r: any) => evidence.push(`Model:${r.name} [${r.category}] ${r.fc || 0} fields`));
        ...
        `);
        graphHits = hits.length;
        hits.forEach((r: any) => evidence.push(`Model:${r.name} fields include "${claim.object}"`));
    ...
    case "integration": {
      ...
      `);
      graphHits = hits.length;
      hits.forEach((r: any) => evidence.push(`Integration:${r.name} [${r.type}] provider:${r.provider || "unknown"}`));
    ...
    case "pattern": {
      ...
      `);
      graphHits = hits.length;
      hits.forEach((r: any) => evidence.push(`Pattern:${r.name} [${r.type}]`));
    ...
    case "flow": {
      ...
      `);
      graphHits = hits.length;
      hits.forEach((r: any) => evidence.push(`Flow:${r.name} [${r.steps || "?"} steps]`));
      break;
    }
    ...
    case "auth": {
      ...
      `);
      graphHits = hits.length;
      hits.forEach((r: any) => evidence.push(`AuthPattern:${r.name} [${r.type}]`));
      ...
      authInteg.forEach((r: any) => evidence.push(`AuthIntegration:${r.name} [${r.type}]`));
      break;
    }
    ...
    case "data": {
      ...
      `);
      graphHits = hits.length;
      hits.forEach((r: any) => evidence.push(`${claim.subject} → ${r.model} via ${r.app}`));
      ...
      if (graphHits === 0) {
        ...
        reverseHits.forEach((r: any) => evidence.push(`${claim.subject} co-occurs with ${r.model} in ${r.app}`));
      }
      break;
    }
    ...
    case "api": {
      ...
      `);
      graphHits = hits.length;
      hits.forEach((r: any) => evidence.push(`APIPattern:${r.name} [${r.type}]`));
      break;
    }
  }

  // Determine status
  let status: "VERIFIED" | "UNVERIFIED" | "CONTRADICTED";
...
async function checkContradiction(
  ...
  if (claim.category === "integration" && claim.predicate === "IS_INTEGRATION") {
    ...
    `);
    if (typeHits.length > 0 && !typeHits.some((r: any) => r.name.toLowerCase().includes(subjectLower))) {
      // Graph knows about databases but not this one
      const alternatives = typeHits.map((r: any) => r.name).join(", ");
      return {
        confidence: 0.3,
  ...
  if (claim.category === "auth" && claim.predicate === "AUTH_METHOD") {
    ...
    if (authHits.length > 0) {
      const graphAuthMethods = authHits.map((r: any) => r.name.toLowerCase());
      if (!graphAuthMethods.some(m => m.includes(claim.object.toLowerCase()))) {
        return {
          confidence: 0.2,  // Low confidence — auth methods are diverse
          reason: `Graph auth integrations are [${authHits.map((r: any) => r.name).join(", ")}], not "${claim.object}"`,
        };
      }
    }
  }
...
export async function validateFacts(
  ...
  let overallVerdict: "PASS" | "PASS_WITH_CONCERNS" | "FAIL";
  let summary: string;
...
async function main() {
  ...
  const byCategory = new Map<string, { verified: number; unverified: number; contradicted: number }>();
  for (const v of result.verdicts) {
    const cat = v.claim.category;
    if (!byCategory.has(cat)) byCategory.set(cat, { verified: 0, unverified: 0, contradicted: 0 });
    const entry = byCategory.get(cat)!;
    if (v.status === "VERIFIED") entry.verified++;
    else if (v.status === "UNVERIFIED") entry.unverified++;
    else entry.contradicted++;
  }
